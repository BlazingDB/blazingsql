...
class TestCreateTableFromGDF(unittest.TestCase):
    ...
    def test_simple_pandas(self):
        ...
        nation_df = pd.read_csv(
            "/home/percy/Blazing/projects/demos/data/nation.psv",
            delimiter='|',
            dtype=column_types,
            names=column_names)
    ...
    def test_simple_arrow(self):
        arrow_table = pa.RecordBatchStreamReader(
            '/home/percy/Blazing/projects/demos/data/gpu.arrow').read_all()
    ...
    def test_simple_parquet(self):
        ...
        # table = self.context.create_table('holas_parquet', fileType =
        # Parquet, '/tpch-1gb/nation/')

        table = self.context.create_table(
            table_name, '/home/percy/Blazing/Parquet/tpch-1gb/nation/0_0_0.parquet')
        result = self.context.sql(
            "select * from main.holas_parquet",
            ["table1"])  # async
        now = result.get()  # data
    ...
    def test_simple_csv(self):
        ...
        table = self.context.create_table(table_name, path,
                                          ...
                                          # TODO percy support string later use
                                          # int64 for now
                                          dtype=[
                                              'int', 'int64', 'int', 'int64'],
                                          delimiter='|',
