...
def main(dask_client, drill, dir_data_file, bc, nRals):
    ...
    def executionTest ():
        ...
        data_types =  [DataType.DASK_CUDF, DataType.CUDF, DataType.CSV, DataType.PARQUET] # TODO json
        ...
        for fileSchemaType in data_types:
            ...
            queryId = 'TEST_04'
            query = "select o_orderpriority, count(*) as order_count from orders group by o_orderpriority"
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)

            queryId = 'TEST_05'
            query = "SELECT o_custkey, count(o_orderkey), count(coalesce(o_orderkey,0)), count(o_custkey), count(*) from orders group by o_custkey"
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)

            queryId = 'TEST_06'
            query = "SELECT c_custkey, count(c_nationkey), min(c_nationkey), sum(c_nationkey)  from customer group by c_custkey"
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
