...
def main(dask_client, drill, dir_data_lc, bc, nRals):
    ...
    def executionTest(): 
        tables = ['nation', 'region', 'supplier', 'customer', 'part', 'lineitem', 'orders', 'partsupp']
        ...
        for fileSchemaType in data_types:
            ...
            queryId = 'TEST_00'
            query = "select count(p_partkey), sum(p_partkey), max(p_partkey), min(p_partkey) from part"
            ...
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, True, fileSchemaType)
            ...
            (select min(p_partkey) as partKey, sum(p_size)/count(p_size) as avgSize, max(p_retailprice) as maxPrice, min(p_retailprice) as minPrice from part ) as partAnalysis
            ...
            #runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType) #TODO: Change sum/count for avg KC    
