...
def main(dask_client, drill, dir_data_file, bc, nRals):
    ...
    def executionTest(): 
        ...
        for fileSchemaType in data_types:
            ...
            queryId = 'TEST_01'
            query = "select c_custkey, c_nationkey, c_acctbal from customer where c_custkey < 15"
            runTest.run_query(bc, drill, query, queryId, queryType, worder, "c_custkey", acceptable_difference, use_percentage, fileSchemaType)
        
            queryId = 'TEST_02'
            query = "select c_custkey, c_nationkey, c_acctbal from customer where c_custkey < 150 and c_nationkey = 5"
            runTest.run_query(bc, drill, query, queryId, queryType, worder, "c_custkey", acceptable_difference, use_percentage, fileSchemaType)
        
            queryId = 'TEST_03'
            query = "select c_custkey, c_nationkey as nkey from customer where c_custkey < 0"
            runTest.run_query(bc, drill, query, queryId, queryType,  worder, "c_custkey", acceptable_difference, use_percentage, fileSchemaType)
            ...
            queryId = 'TEST_05'
            query = "select c_custkey, c_nationkey as nkey from customer where c_custkey < 0 or c_nationkey >= 24"
            runTest.run_query(bc, drill, query, queryId, queryType,  worder, "c_custkey", acceptable_difference, use_percentage, fileSchemaType)
        
            queryId = 'TEST_06'
            query = "select c_custkey, c_nationkey as nkey from customer where c_custkey < 0 and c_nationkey >= 3"
            runTest.run_query(bc, drill, query, queryId, queryType,  worder, "c_custkey", acceptable_difference, use_percentage, fileSchemaType)
        ...
        #     runTest2.run_query(bc, drill, query, queryId, queryType,  worder, "c_custkey", acceptable_difference, use_percentage, fileSchemaType)
        # 
        ...
        #     runTest2.run_query(bc, drill, query, queryId, queryType,  worder, "c_custkey", acceptable_difference, use_percentage, fileSchemaType)
            ...
            queryId = 'TEST_07'
            query = "select c_custkey, c_nationkey, c_acctbal from customer where c_custkey < 150 and c_nationkey = 5 and c_acctbal > 600"
            runTest.run_query(bc, drill, query, queryId, queryType,  worder, "c_custkey", acceptable_difference, use_percentage, fileSchemaType)
        
            queryId = 'TEST_08'
            query = "select c_custkey, c_nationkey as nkey from customer where c_custkey > 0 or c_nationkey >= 24 or c_acctbal > 700"
