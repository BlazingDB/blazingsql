...
def main(dask_client, drill, dir_data_file, bc, nRals):
    ...
    def executionTest(): 
        ...
        for fileSchemaType in data_types:
            ...
            queryId = 'TEST_01'
            query = """select MIN(n.n_nationkey), MAX(r.r_regionkey), AVG(CAST((n.n_nationkey + r.r_regionkey) AS DOUBLE)) 
                from nation as n left outer join region as r on n.n_nationkey = r.r_regionkey where n.n_nationkey IS NULL"""
            ...
            queryId = 'TEST_02'
            query = """select SUM(n1.n_nationkey) as n1key, AVG(CAST((n2.n_nationkey +  n1.n_nationkey) AS DOUBLE)) as n2key 
                from nation as n1 full outer join nation as n2 on n1.n_nationkey = n2.n_nationkey + 10 where n1.n_nationkey IS NOT NULL"""
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', 0.01, use_percentage, fileSchemaType)
         
            queryId = 'TEST_03'
            query = """select COUNT(n1.n_nationkey) as n1key, COUNT(n2.n_nationkey +  n1.n_nationkey) as n2key 
            from nation as n1 full outer join nation as n2 on n1.n_nationkey = n2.n_nationkey + 10 where n1.n_nationkey IS NOT NULL"""
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
         
            queryId = 'TEST_04'
            query = """select COUNT(n1.n_regionkey), AVG(CAST(n1.n_regionkey AS DOUBLE)) 
