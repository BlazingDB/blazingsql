...
def main(dask_client, drill, dir_data_lc, bc, nRals):
    ...
    def executionTest ():
        ...
        data_types =  [DataType.CSV, DataType.ORC, DataType.PARQUET] # TODO parquet json

        for fileSchemaType in data_types:
            if skip_test(dask_client, nRals, fileSchemaType, queryType): continue
            cs.create_tables(bc, hdfs_dir_data_lc, fileSchemaType, tables=tables)
        ...
        #   Run Query -----------------------------------------------------------------------------
            ...
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', 0.01, True, fileSchemaType) #TODO: Change sum/count for avg KC
