...
from pynvml import *
from Utils import dquery, init_context, skip_test
...
def main(dask_client, drill, dir_data_file, bc, nRals):
    ...
    def executionTest ():
        tables = ['partsupp', 'lineitem', 'part', 'supplier', 'orders', 'customer', 'region', 'nation']
        ...
        for fileSchemaType in data_types:
            ...
            queryId = 'TEST_01'
            query = "select count(distinct (n_regionkey + n_nationkey)), n_regionkey from nation group by n_regionkey"
            runTest.run_query(bc, drill, query, queryId, queryType, worder, "n_regionkey", acceptable_difference, use_percentage, fileSchemaType)

            queryId = 'TEST_02'
            query = "select count(distinct o_custkey), o_orderkey from orders where o_orderkey<100 group by o_orderkey, (o_orderkey + o_custkey)"
            runTest.run_query(bc, drill, query, queryId, queryType, worder, "o_orderkey", acceptable_difference, use_percentage, fileSchemaType)
        
            queryId = 'TEST_03'
            query = "select count(distinct(o_orderkey + o_custkey)) as new_col, sum(o_orderkey), o_custkey from orders group by o_custkey"
            runTest.run_query(bc, drill, query, queryId, queryType, worder, "o_custkey", acceptable_difference, use_percentage, fileSchemaType)
        
            # queryId = 'TEST_04'
            # query = "select count(distinct(o_custkey)), avg(o_totalprice), (o_orderkey + o_custkey) as num from orders where o_custkey < 100 group by o_custkey, o_orderkey"
            # runTest.run_query(bc, drill, query, queryId, queryType, worder, '', 0.01, use_percentage, fileSchemaType)
            
            queryId = 'TEST_05'
            query = "select count(distinct(o_custkey)), max(o_totalprice), min(o_totalprice), avg(o_totalprice) from orders group by o_custkey"
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', 0.01, use_percentage, fileSchemaType)
        
            queryId = 'TEST_06'
            query = "select n_nationkey, count(distinct (n_regionkey + n_nationkey))/count(n_nationkey) from nation group by n_nationkey"
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', 0.01, use_percentage, fileSchemaType)
