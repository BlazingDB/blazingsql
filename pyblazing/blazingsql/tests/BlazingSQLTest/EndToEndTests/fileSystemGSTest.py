...
queryType = 'File System Google Storage '  
...
def main(dask_client, drill, dir_data_lc, bc, nRals):
    ...
    def executionTest(): 
        ...
        #lineitem_files = ['gcs://'+ authority +'/100MB2Part/']
        
        tables = ['nation', 'region', 'supplier','customer','lineitem','orders']
        data_types =  [DataType.CSV, DataType.PARQUET] # TODO json DataType.ORC,
        ...
        #   Run Query -----------------------------------------------------------------------------
            ...
            where n1.n_nationkey = 1 and n2.n_nationkey = 2 and o.o_orderkey < 20"""
            runTest.run_query(bc,drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
