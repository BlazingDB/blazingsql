...
def main(dask_client, drill, spark, dir_data_file, bc, nRals):
    ...
    def executionTest (queryType):
        ...
        data_types =  [DataType.DASK_CUDF, DataType.CUDF, DataType.CSV, DataType.ORC, DataType.PARQUET] # TODO ORC gdf parquet json
...
#         
        #Run Query -----------------------------------------------------------------------------
            ...
            queryId = 'TEST_01'
            query = "select n_regionkey as rkey, n_nationkey from nation where n_regionkey < 3"
            ...
            queryId = 'TEST_02'
            query = "select n_nationkey, n_regionkey as nr from nation where n_regionkey < 3"
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
             
            queryId = 'TEST_03'
            query = "select n_regionkey as rkey, n_nationkey from nation where n_regionkey < 3 and n_nationkey > 5"
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
             
            queryId = 'TEST_04'
            query = "select n_nationkey as nkey, n_comment from nation where n_nationkey > 5"
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
            ...
            queryId = 'TEST_06'
            query = "select n_regionkey as rkey, n_nationkey as nkey from nation"
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
             
            queryId = 'TEST_07'
            query = "select n_nationkey as nkey, n_regionkey as rkey from nation"
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
    
            queryId = 'TEST_08'
            query = """(select r_name as name, r_regionkey as key from region ) 
                        union all (select n_name as name, n_nationkey as key from nation )"""
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
             
            queryId = 'TEST_09'
            query = """select o_orderkey as okey, o_custkey as ckey, o_orderdate as odate from orders 
                       where o_orderpriority = '1-URGENT' order by okey, ckey, odate"""
            ...
            queryId = 'TEST_11'
            query = """select count(o_orderkey) as count_ok from orders  group by o_orderstatus"""
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
