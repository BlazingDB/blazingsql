...
def main(dask_client, drill, spark, dir_data_file, bc, nRals):
    ...
    def executionTest(): 
        ...
        for fileSchemaType in data_types:
            ...
            queryId = 'TEST_01'
            query = """select c_mktsegment || ': ' || c_custkey || ' - ' || c_name from customer 
            ...
            queryId = 'TEST_03'        
            query = """select c.c_name || ' ' || o.o_orderkey, o.o_orderstatus from orders o 
            ...
            query = """select c.c_name, o.o_orderkey, o.o_orderstatus from orders o 
                        inner join customer c on o.o_custkey = c.c_custkey
                        where 'Customer#000000' || c.c_custkey like 'Customer#0000001'"""
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
            ...
            queryId = 'TEST_06'        
            query = """select o.o_orderkey || c.c_name , o.o_orderstatus from orders o 
                       inner join customer c on o.o_custkey = c.c_custkey
                       where c.c_custkey < 10"""
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
            
            queryId = 'TEST_07'        
            query = """select o.o_orderkey, c.c_name || cast(c.c_custkey as VARCHAR), c.c_name || '-' || cast(c.c_custkey as VARCHAR), o.o_orderstatus 
                        from orders o 
                       inner join customer c on o.o_custkey = c.c_custkey
                       where c.c_custkey < 10"""
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
            ...
            queryId = 'TEST_08'        
            query = """select c.c_name || ': ' || c.c_custkey, c.c_name || ': ' || c.c_comment from customer c
                       where c.c_custkey < 10"""
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
            ...
            queryId = 'TEST_10'        
            query = """select * from (
                        select c.c_custkey, 'Customer#000000' || c.c_custkey as n_name from customer c
                        where c.c_custkey < 10
                    ) as n where n.n_name = 'Customer#000000' || n.c_custkey"""
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
            
            queryId = 'TEST_11'        
            query = """select c.c_custkey, c.c_name || '- ' || c.c_custkey, c.c_comment from customer c
                       where c.c_custkey < 0"""
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
