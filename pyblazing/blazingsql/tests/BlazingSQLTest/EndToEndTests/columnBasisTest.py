...
def main(dask_client, drill, dir_data_file, bc, nRals):
    ...
    def executionTest ():
        ...
        for fileSchemaType in data_types:
            ...
            query = "select c_custkey + 3, c_nationkey, c_acctbal from customer"
         
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType, nested_query=True, blz_result = result_gdf, print_result=True)
            # End Test 66------------------------------------------------------------------------------
            ...
            query = "select c_custkey, c_nationkey, c_acctbal from customer where c_acctbal > 1000"
            query_blz = runTest.get_blazingsql_query('main', query)
            ...
            query = "select c_nationkey, c_acctbal + 3 from customer where c_acctbal > 1000"
            ...
            query = "select c_acctbal + 3 as c_acctbal_new from customer where c_acctbal > 1000"
            ...
            query = "select c_acctbal, c_acctbal + 3 as c_acctbal_new from customer where c_acctbal > 1000"
            ...
            token = bc.sql("select n_nationkey, n_regionkey from nation where n_nationkey < 0")
            result_gdf = get_results(nRals, token)
        
            bc.create_table('results', result_gdf)
        
            token1 = bc.sql("select n.n_nationkey, r.n_regionkey from main.nation as n left join main.results as r on n.n_nationkey = r.n_nationkey")  
            ...
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType, nested_query=True, blz_result = result_gdf1)
            # End Test 71------------------------------------------------------------------------------
            ...
            result_gdf['n_nationkey'] = result_gdf['n_nationkey'].astype('int32')
            ...
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType, nested_query=True, blz_result = result_gdf1, print_result=True)
