...
def initializeBlazing(
    ...
    if allocator not in possible_allocators:
        print(
            'ERROR: parameter "allocator" was not set to a proper value. It was set to: '
            + str(allocator)
    ...
    for table_index in range(len(tables)):
        if isinstance(tables[table_index].input, dask_cudf.core.DataFrame):
            if single_gpu:
                ...
                print(
                    "ERROR: collectPartitionsRunQuery should not be called with an input of dask_cudf.core.DataFrame"
                )
                logging.error(
                    "collectPartitionsRunQuery should not be called with an input of dask_cudf.core.DataFrame"
                )
...
def mergeTableScans(tableScanInfo):
    ...
    for index, table_name in enumerate(tableScanInfo["table_names"]):
        if (
            len(tableScanInfo["table_columns"][index]) > 0
        ):  # if the column list is empty, it means we want all columns
...
def parseHiveMetadata(curr_table, uri_values):
    ...
    final_names = (
        []
    )  # not all columns will have hive metadata, so this vector will capture all the names that will actually be used in the end
...
def mergeMetadata(curr_table, fileMetadata, hiveMetadata):
    ...
    if not fileMetadata["file_handle_index"].equals(hiveMetadata["file_handle_index"]):
        print(
            "ERROR: file_handle_index of fileMetadata does not match the same order as in hiveMetadata"
        )
        return hiveMetadata
...
def distributed_initialize_server_directory(client, dir_path):
    ...
    host_list = list(
        set(
            [value["host"] for key, value in client.scheduler_info()["workers"].items()]
        )
    )
    initialized = {}
...
class BlazingTable(object):
    def __init__(
        ...
        if self.fileType == DataType.CUDF:
            ...
            self.column_types = [
                cio.np_to_cudf_types_int(x.dtype) for x in self.input._data.values()
            ]
...
class BlazingContext(object):
    ...
    def __init__(
        ...
        logging_dir_path = "blazing_log"
        if (
            "BLAZING_LOGGING_DIRECTORY" in config_options
        ):  # want to use config_options and not self.config_options since its not encoded
        ...
        if (
            not "BLAZING_HOST_MEM_RESOURCE_CONSUMPTION_THRESHOLD".encode()
            in self.config_options
        ):
            self.config_options[
                "BLAZING_HOST_MEM_RESOURCE_CONSUMPTION_THRESHOLD".encode()
            ] = str(host_memory_quota).encode()
            ...
            self.config_options[
                ...
                * len(
                    set(
                        [
                            value["host"]
                            for key, value in self.dask_client.scheduler_info()[
                                "workers"
                            ].items()
                        ]
                    )
                )
                / len(self.dask_client.scheduler_info()["workers"])
            ).encode()
    ...
    def create_table(self, table_name, input, **kwargs):
        ...
        user_partitions = kwargs.get(
            "partitions", None
        )  # these are user defined partitions should be a dictionary object of the form partitions={'col_nameA':[val, val], 'col_nameB':['str_val', 'str_val']}
        if user_partitions is not None and type(user_partitions) != type({}):
            print(
                "ERROR: User defined partitions should be a dictionary object of the form partitions={'col_nameA':[val, val], 'col_nameB':['str_val', 'str_val']}"
            )
            logging.error(
                "ERROR: User defined partitions should be a dictionary object of the form partitions={'col_nameA':[val, val], 'col_nameB':['str_val', 'str_val']}"
            )
            return
        user_partitions_schema = kwargs.get(
            "partitions_schema", None
        )  # for user defined partitions, partitions_schema should be a list of tuples of the column name and column type of the form partitions_schema=[('col_nameA','int32','col_nameB','str')]
        if user_partitions_schema is not None:
            if user_partitions is None:
                print(
                    "ERROR: 'partitions_schema' was defined, but 'partitions' was not. The parameter 'partitions_schema' is only to be used when defining 'partitions'"
                )
                logging.error(
                    "ERROR: 'partitions_schema' was defined, but 'partitions' was not. The parameter 'partitions_schema' is only to be used when defining 'partitions'"
                )
                return
            elif type(user_partitions_schema) != type([]) and all(
                len(part_schema) == 2 for part_schema in user_partitions_schema
            ):
                print(
                    "ERROR: 'partitions_schema' should be a list of tuples of the column name and column type of the form partitions_schema=[('col_nameA','int32','col_nameB','str')]"
                )
                logging.error(
                    "ERROR: 'partitions_schema' should be a list of tuples of the column name and column type of the form partitions_schema=[('col_nameA','int32','col_nameB','str')]"
                )
        ...
        if isinstance(input, hive.Cursor):
            ...
            if file_format_hint == "undefined":
                ...
                print(
                    ...
                    + ") does not match the file_format infered by the Hive cursor ("
            ...
            if user_partitions_schema is None:
                print(
                    "ERROR: When using 'partitions' without a Hive cursor, you also need to set 'partitions_schema' which should be a list of tuples of the column name and column type of the form partitions_schema=[('col_nameA','int32','col_nameB','str')]"
                )
                logging.error(
                    "ERROR: When using 'partitions' without a Hive cursor, you also need to set 'partitions_schema' which should be a list of tuples of the column name and column type of the form partitions_schema=[('col_nameA','int32','col_nameB','str')]"
                )
            ...
            if isinstance(input, str):
                ...
                print(
                    "ERROR: When using 'partitions' without a Hive cursor, the input needs to be a path to the base folder of the partitioned data"
                )
                logging.error(
                    "ERROR: When using 'partitions' without a Hive cursor, the input needs to be a path to the base folder of the partitioned data"
                )
        ...
        if isinstance(input, cudf.DataFrame):
            ...
            ignore_missing_paths = (
                user_partitions_schema is not None
            )  # if we are using user defined partitions without hive, we want to ignore paths we dont find.
            ...
            if is_hive_input:
                table.column_names = hive_schema[
                    "column_names"
                ]  # table.column_names are the official schema column_names
                table.file_column_names = parsedSchema[
                    "names"
                ]  # table.file_column_names are the column_names used by the file (may be different)
                merged_types = []
                if len(hive_schema["column_types"]) == len(parsedSchema["types"]):
                    for i in range(len(parsedSchema["types"])):
                        if (
                            parsedSchema["types"][i] == 0
                        ):  # if the type parsed from the file is 0 we want to use the one from Hive
                            merged_types.append(hive_schema["column_types"][i])
                        else:
                            merged_types.append(parsedSchema["types"][i])
                else:
                    print(
                        "ERROR: number of hive_schema columns does not match number of parsedSchema columns"
                    )
                    logging.error(
                        "ERROR: number of hive_schema columns does not match number of parsedSchema columns"
                    )
                ...
                table.column_names = parsedSchema[
                    "names"
                ]  # table.column_names are the official schema column_names
                table.file_column_names = parsedSchema[
                    "names"
            ...
            if parsedSchema["file_type"] == DataType.PARQUET:
                ...
                if len(file_groups) != len(table.files):
                    table.metadata, table.files = adjust_due_to_missing_rowgroups(
    ...
    def _parseMetadata(self, file_format_hint, currentTableNodes, schema, kwargs):
        if self.dask_client:
            ...
            for worker_id, worker in enumerate(workers):
                file_subset = [
                    file.decode() for file in currentTableNodes[worker_id].files
                ]
    ...
    def _optimize_with_skip_data_getSlices(
    ...
    def sql(
        ...
        if "LogicalValues(tuples=[[]])" in algebra:
            print(
                "This SQL statement returns empty result. Please double check your query."
            )
        ...
        if self.dask_client is None or single_gpu == True:
            ...
            worker = tuple(self.dask_client.scheduler_info()["workers"])[0]
            connection = self.dask_client.submit(
                cio.getTableScanInfoCaller, algebra, self.tables, workers=[worker]
            )
        ...
        for table_idx, query_table in enumerate(query_tables):
            ...
            if (
                ...
                if query_table.has_metadata():
                    currentTableNodes = self._optimize_with_skip_data_getSlices(
