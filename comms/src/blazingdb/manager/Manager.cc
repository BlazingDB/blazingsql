...
public:
    ...
    thread = std::thread([this]() {
      while(socket.connected()){
        try {
          ...
          memcpy((char *)&address_metadata, request.data(), sizeof(Address::MetaData));
  ...
  Context *generateContext(std::string logicalPlan, int clusterSize, uint32_t context_token) final {
    auto taskNodes = cluster_.getAvailableNodes(clusterSize);
    if (taskNodes.size() > 0) {
      runningTasks_.push_back(std::make_unique<Context>(context_token, taskNodes, taskNodes.at(0), logicalPlan));
    } else {
      std::cerr << "CERR: Context with null taskNode\n";
      runningTasks_.push_back(std::make_unique<Context>(context_token, taskNodes, nullptr, logicalPlan));
    }
...
public:
  ...
  Status Send(Message &message) override {
    ...
    zmq::message_t request((void*)&address_metadata, sizeof (Address::MetaData));
    socket.send (request);

    zmq::message_t reply;
    socket.recv(&reply);
    auto ok_message = std::string(static_cast<char*>(reply.data()), reply.size());
...
std::unique_ptr<ManagerClient> Manager::MakeClient(const std::string &ip, uint16_t port) {
