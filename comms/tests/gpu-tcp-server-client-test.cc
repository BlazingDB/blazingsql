...
static std::string serializeToBinary(std::vector<gdf_column*>& columns) {
  ...
  for (auto column : columns) {
    if (!GpuFunctions::isGdfString(column)) {
      capacity +=
          GpuFunctions::getDataCapacity(column);
      capacity +=
          GpuFunctions::getValidCapacity(column);
    }
  }
  ...
  std::cout << "checksum:"  << std::hex << std::uppercase << crc_result.checksum() << std::endl;
...
class GPUComponentMessage : public GPUMessage {
 public:
  GPUComponentMessage(
      uint32_t contextToken, std::shared_ptr<Node> &sender_node,
  ...
  auto get_raw_pointers(gdf_column* column) {
    ...
    NVStrings *nvStrings_ = category->gather_strings(static_cast<nv_category_index_type *>(column->data), category->size(), true);
    if (!nvStrings_) {
    ...
    auto stringsSize_ =
        std::accumulate(lengthPerStrings, lengthPerStrings + stringsLength_,
                        0, [](int accumulator, int currentValue) {
              return accumulator + std::max(currentValue, 0);
            });
    gdf_size_type offsetsSize_ = offsetsLength_ * sizeof(int);
    char* stringsPointer_;
    int* offsetsPointer_;
    RMM_ALLOC(reinterpret_cast<void **>(&stringsPointer_), stringsSize_ * sizeof(char), 0);
    RMM_ALLOC(reinterpret_cast<void **>(&offsetsPointer_), offsetsSize_ * sizeof(int), 0);
    gdf_size_type nullMaskSize_ = 0;
    unsigned char *nullBitmask_ = nullptr;
    if (null_count > 0) {
      nullMaskSize_ = blazingdb::test::get_bitmask_size_in_bytes(stringsLength_);
      RMM_ALLOC(reinterpret_cast<void **>(&nullBitmask_), nullMaskSize_ * sizeof(unsigned char), 0);
    }
    nvStrings_->create_offsets(stringsPointer_, offsetsPointer_, nullBitmask_, true);
    size_t totalSize_ =
        stringsSize_ + offsetsSize_ + nullMaskSize_ + 4 * sizeof(const std::size_t);
    delete[] lengthPerStrings;
    return std::make_tuple(stringsPointer_, stringsSize_, offsetsPointer_, offsetsSize_, nullBitmask_, nullMaskSize_);
  }
  ...
  virtual raw_buffer GetRawColumns() override {
    ...
    for (int i = 0; i < samples.size(); ++i) {
      ...
      std::cout << "column:" << i << "|" << col_transport.metadata.dtype  << "|" << col_transport.metadata.size << "|" << col_transport.metadata.null_count  << "|" <<col_transport.metadata.col_name << std::endl;
      if (blazingdb::test::GpuFunctions::isGdfString(column)) {
        ...
        buffer_sizes.push_back(blazingdb::test::GpuFunctions::getDataCapacity(column));
        raw_buffers.push_back((char*)column->data);
        col_transport.valid = raw_buffers.size();
        buffer_sizes.push_back(blazingdb::test::GpuFunctions::getValidCapacity(column));
  ...
  MakeFrom(const Message::MetaData &message_metadata,
    ...
    auto node = std::make_shared<Node>(Address::TCP(address_metadata.ip,
    ...
    for (size_t i = 0; i < num_columns; ++i) {
      ...
      std::cout << "column:" << i << "|" << columns_offsets[i].metadata.dtype  << "|" << columns_offsets[i].metadata.size << "|" << columns_offsets[i].metadata.null_count  << "|" <<columns_offsets[i].metadata.col_name << std::endl;
      ...
      if (string_offset != -1) {
        ...
        received_samples[i] = blazingdb::test::create_nv_category_gdf_column(nvcategory_ptr, columns_offsets[i].metadata.size, (char*)columns_offsets[i].metadata.col_name);
      } else {
        ...
        gdf_error err = gdf_column_view_augmented(received_samples[i],
                                                  ...
                                                  gdf_dtype_extra_info{
                                                      .time_unit = (gdf_time_unit) columns_offsets[i].metadata.time_unit,
                                                      .category = nullptr
                                                  },
...
std::shared_ptr<GPUMessage>
CreateSampleToNodeMaster(uint32_t context_token, std::shared_ptr<Node> &sender_node) {
  ...
  std::tie(buffer_sizes, buffers, column_offsets) = gpu_message->GetRawColumns();
  for (auto &info : column_offsets) {
      std::cout << "\t " << info.strings_data << "|" << info.strings_offsets << info.data << "\n";
  }
  ...
  for (int index=0; index < buffer_sizes.size(); index++) {
    ...
    cudaMemcpy(result.data(), gpu_buffer, gpu_buffer_sz, cudaMemcpyDeviceToHost);
    boost::crc_32_type  crc_result;
    crc_result.process_bytes( (char*)result.data(), result.size() );
    std::cout << "\tchecksum_col(" << index << "):" << std::hex << std::uppercase << crc_result.checksum() << std::endl;
  }
  auto gpu_message_copy = std::dynamic_pointer_cast<GPUComponentMessage>(GPUComponentMessage::MakeFrom(gpu_message->metadata(), gpu_message->getSenderNode()->address()->metadata(), column_offsets, buffers)) ;
