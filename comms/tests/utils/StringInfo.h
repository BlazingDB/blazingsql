...
public:
  ...
  public:
    explicit StringInfo(gdf_column *column)
        : nvStrings_{nullptr},
          stringsLength_{0},
          offsetsLength_{0},
          stringsPointer_{nullptr},
          offsetsPointer_{nullptr},
          nullBitmask_{nullptr},
          stringsSize_{0},
          offsetsSize_{0},
          nullMaskSize_{0},
          totalSize_{0} {
      NVCategory *nvCategory =
          reinterpret_cast<NVCategory *>(column->dtype_info.category);
      if (!nvCategory) {
        nvCategory = NVCategory::create_from_array(nullptr, 0);
      }
      nvStrings_ = nvCategory->gather_strings(
          static_cast<nv_category_index_type *>(column->data), column->size,
          true);
      ...
      if (column->null_count > 0) {
        // get_bitmask_size_in_bytes is padded to 64 bytes but only need
        // (stringsLength_ + 7) / 8
      ...
      nvStrings_->create_offsets(stringsPointer_, offsetsPointer_, nullBitmask_,
                                 false);
      totalSize_ = stringsSize_ + offsetsSize_ + nullMaskSize_ +
  ...
  ~StringsInfo() {
    std::for_each(columnMap_.cbegin(), columnMap_.cend(),
  ...
  std::size_t capacity() const noexcept {
    return std::accumulate(
        columnMap_.cbegin(), columnMap_.cend(), 0,
        [](int &accumulator,
           const std::pair<gdf_column *const, StringInfo *> &pair) {
