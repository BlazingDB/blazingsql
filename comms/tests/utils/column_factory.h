...
gdf_valid_type * allocate_valid(gdf_column * col){
  ...
  auto allocated_size_valid = gdf_valid_allocation_size(num_values); //so allocations are supposed to be 64byte aligned
...
static gdf_column* create_nv_category_gdf_column(NVCategory* category, size_t num_values,  char *str){
  ...
  column->dtype_info.time_unit = TIME_UNIT_NONE; // TODO this should not be hardcoded
  ...
  if (category->has_nulls()) {
    ...
    column->null_count = category->set_null_bitarray((gdf_valid_type*)column->valid);
  }
  if (str)
    column->col_name = str;
  return column;
...
static cudf::table transfor_to_nv_category(cudf::table & input){
  ...
  for (size_t i = 0; i < input.num_columns(); i++) {
    ...
    if (col->dtype == GDF_STRING){
      ...
      auto nv_col = create_nv_category_gdf_column(category, col->size, col->col_name);
