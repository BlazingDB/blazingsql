...
conda install --yes -c rapidsai-nightly -c nvidia -c conda-forge -c defaults dask-cuda=0.16 dask-cudf=0.16 ucx-py=0.16 ucx-proc=*=gpu cudf=0.16 python=3.7 cudatoolkit=$CUDA_VERSION
...
conda activate bsql
conda install --yes -c conda-forge spdlog=1.7.0 google-cloud-cpp ninja
conda install --yes -c rapidsai-nightly -c nvidia -c conda-forge -c defaults dask-cuda=0.16 dask-cudf=0.16 ucx-py=0.16 ucx-proc=*=gpu cudf=0.16 python=3.7 cudatoolkit=10.1
conda install --yes -c conda-forge cmake gtest gmock cppzmq cython=0.29 openjdk=8.0 maven thrift=0.13.0 jpype1 netifaces pyhive
```
