/*
 * This file is a copy with some modifications of the ReduceExpressionsRule class from
 ...
 * The changes are about avoiding to replace expressions with InputRefs that may cause issues on our engine.
 */
...
public abstract class ReduceExpressionsRule extends RelOptRule
  ...
  protected static class ReducibleExprLocator extends RexVisitorImpl<Void> {
    ...
    @Override public Void visitInputRef(RexInputRef inputRef) {
      ...
      if (constant != null) {
        if (constant instanceof RexCall) {
          ...
          stack.add(Constancy.IRREDUCIBLE_CONSTANT); //Forcing to avoid replacing of a RexInputRef
        }
        return null;
      }
      return pushVariable();
    ...
    private void analyzeCall(RexCall call, Constancy callConstancy) {
      ...
      //Again we force to avoid replacing whether exists a RexInputRef inside the expression
      if (callConstancy == Constancy.REDUCIBLE_CONSTANT) {
        for(RexNode v : call.getOperands()){
            if(v instanceof RexInputRef){
