/*
 * This file is a copy with some modifications of the ReduceExpressionsRule from
 * the Apache Calcite project. The original code can be found at:
 * https://github.com/apache/calcite/blob/branch-1.23/core/src/main/java/org/apache/calcite/rel/rules/ReduceExpressionsRule.java
 * The changes are about to avoid Reduce expression when window functions exists
 */
...
public abstract class ReduceExpressionsRule extends RelOptRule
   ...
   */
  // This is the Rule we want to chnage in order to avoid Reduced expressions when exists Window Functions
  public static class ProjectReduceExpressionsRule extends ReduceExpressionsRule {
    ...
    // This is the method we want to chnage in order to avoid Reduced expressions when exists Window Functions
    @Override public void onMatch(RelOptRuleCall call) {
        ...
        // we want to avoid Reduced expressions when exists Window Functions
        for (int i = 0; i < expList.size(); i++) {
            if (expList.get(i).toString().indexOf("OVER") == -1 ) {
                expList.remove(i);
                i--;
            }
        }
