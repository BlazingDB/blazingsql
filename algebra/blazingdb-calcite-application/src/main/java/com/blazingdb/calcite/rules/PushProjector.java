 ...
 * The changes are about creating a map to maintain the column aliases that will be used on the ProjectFilterTransposeRule.
 */
  ...
  public Project createProjectRefsAndExprs(
    ...
    for (int i = 0; i < nInputRefs; i++) {
        if(i < origProj.getChildExps().size()) {
            fieldsMap.put(origProj.getChildExps().get(i).hashCode(),
            fieldNames.get(i));
        }
    }
