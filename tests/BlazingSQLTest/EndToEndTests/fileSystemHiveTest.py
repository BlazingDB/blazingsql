...
from Utils import gpuMemory, test_name, init_context, skip_test, startHadoop
from pynvml import *
...
def main(dask_client, drill, dir_data_lc, bc, nRals):
    ...
    def executionTest ():
        ...
        result, error_msg, fs = bc.hdfs(authority, host=hdfs_host, port=hdfs_port, user='jhs', driver=hdfs_driver, kerb_ticket=krbticket)

        if result == False:
            msg = "WARNING: Could not connect to HDFS instance %s:%d using driver %s, error was: %s" % (hdfs_host, hdfs_port, hdfs_driver, error_msg)
            print(msg)
        ...
        #tables = ['customer']
        data_types =  [DataType.CSV, DataType.ORC, DataType.PARQUET] # TODO json

        for fileSchemaType in data_types:
            #if skip_test(dask_client, nRals, fileSchemaType, queryType): continue
            cs.create_hive_tables(bc, hdfs_dir_data_lc, fileSchemaType, tables=tables)
        ...
        #   Run Query -----------------------------------------------------------------------------
            ...
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', 0.01, True) #TODO: Change sum/count for avg KC
...
if __name__ == '__main__':
    ...
    cs.init_hive_schema(cursor, Settings.data['TestSettings']['dataDirectory'])
