...
def main(drill, dir_data_lc, bc, nRals):
    ...
    def executionTest(queryType):  
        #Register HDFS 
        bc.hdfs("35.185.48.245", host="35.185.48.245", port=54310, user='hadoop')
        ...
        bc.s3(authority, bucket_name='blazingsql-bucket', encryption_type=EncryptionType.NONE,
              access_key_id='', secret_key='')
        dir_df = dir_data_lc[dir_data_lc.find("DataSet"):len(dir_data_lc)]
        dir_data_fs_s3 = "s3://" + authority + "/" + dir_df
        
        #parque local
        region_files = cs.get_filenames_for_table('region', dir_data_lc, 'parquet', '')
        bc.create_table('region', region_files)
        #csv local
        nation_files = cs.get_filenames_for_table('nation', dir_data_lc, 'psv', '')
        bc.create_table('nation', nation_files, delimiter='|', dtype=cs.get_dtypes('nation'), names=cs.get_column_names('nation'))
        #parquet from hdfs
        lineitem_files = cs.get_filenames_for_table('lineitem', dir_data_lc, 'parquet', dir_data_fs_hdfs)
        bc.create_table('lineitem', lineitem_files)
        #parquet from S3  
        customer_files = cs.get_filenames_for_table('customer', dir_data_lc, 'parquet', '')
        bc.create_table('customer', customer_files)
        #csv con hdfs  
        supplier_files = cs.get_filenames_for_table('supplier', dir_data_lc, 'psv', dir_data_fs_hdfs)
        bc.create_table('supplier', supplier_files, delimiter='|', dtype=cs.get_dtypes('supplier'), names=cs.get_column_names('supplier'))
        #csv from S3
        orders_files = cs.get_filenames_for_table('orders', dir_data_lc, 'psv', dir_data_fs_s3)
        bc.create_table('orders', orders_files, delimiter='|', dtype=cs.get_dtypes('orders'), names=cs.get_column_names('orders'))
        ...
        query = """select sum(c.c_custkey)/count(c.c_custkey), sum(c.c_acctbal)/count(c.c_acctbal), n.n_nationkey, r.r_regionkey from customer as c 
        ...
        runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, True) #TODO: Change sum/count for avg KC
        ...
        query = """select o.o_orderkey, o.o_totalprice, l.l_partkey, l.l_returnflag from orders as o
                inner join lineitem as l on o.o_orderkey = l.l_orderkey where l.l_orderkey < 1000"""
        ...
        inner join nation as n on c.c_nationkey = n.n_nationkey where n.n_regionkey = 1 and c.c_custkey < 50"""
        runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
        ...
        inner join nation as n2 on c.c_nationkey = n2.n_nationkey where n1.n_nationkey = 1 and n2.n_nationkey = 2 and o.o_orderkey < 10000"""
        runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
