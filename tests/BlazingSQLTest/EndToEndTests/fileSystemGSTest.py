...
def main(dask_client, drill, dir_data_lc, bc, nRals):
    ...
    def executionTest():
        ...
        googleStorageProjectId = Settings.data["TestSettings"][
                                               "googleStorageProjectId"]
        googleStorageBucketName = Settings.data["TestSettings"][
            "googleStorageBucketName"
        ]
        googleStorageAdcJsonFile = Settings.data["TestSettings"][
            "googleStorageAdcJsonFile"
        ]
        ...
        for fileSchemaType in data_types:
            ...
            queryId = "TEST_04"
            query = """select count(c_custkey), sum(c_acctbal), SUM(c_acctbal)
                    / count(c_acctbal), min(c_custkey), max(c_nationkey),
