...
def main(dask_client, drill, dir_data_lc, bc, nRals):
    ...
    def executionTest(queryType): 
        ...
        dir_data_lc = "s3://" + authority + "/" + "DataSet100Mb2part/" #dir_df
        ...
        #   Run Query -----------------------------------------------------------------------------
            ...
            queryId = 'TEST_04'
            query = """select count(c_custkey), sum(c_acctbal), sum(c_acctbal)/count(c_acctbal), min(c_custkey), 
                    max(c_nationkey), (max(c_nationkey) + min(c_nationkey))/2 c_nationkey from customer 
            ...
            where n1.n_nationkey = 1 and n2.n_nationkey = 2 and o.o_orderkey < 30"""
            runTest.run_query(bc,drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
            ...
            queryId = 'TEST_14'
            query = """select 100168549 - sum(o_orderkey)/count(o_orderkey), 56410984/sum(o_totalprice), 
            (123 - 945/max(o_orderkey))/(sum(81619/o_orderkey)/count(81619/o_orderkey)) from orders where o_orderkey < 50"""
            runTest.run_query(bc,drill, query, queryId, queryType, worder, '', acceptable_difference, True, fileSchemaType)   #TODO: Change sum/count for avg KC    
