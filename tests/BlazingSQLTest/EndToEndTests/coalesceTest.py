...
from Utils import gpuMemory, dquery
from pynvml import *
...
def main(dask_client, drill, dir_data_file, bc, nRals):
    ...
    def executionTest (queryType):
        ...
        data_types =  [DataType.DASK_CUDF, DataType.CSV, DataType.PARQUET] # TODO json
        ...
        for fileSchemaType in data_types:
            ...
            queryId = 'TEST_01'
            query = "select n.n_nationkey, COALESCE(r.r_regionkey,-1) from nation as n left outer join region as r on n.n_nationkey = r.r_regionkey where n.n_nationkey < 10"
            ...
            queryId = 'TEST_04'
            query = "select customer.c_custkey, orders.o_orderkey, COALESCE(orders.o_custkey,123456) from customer left outer join orders on customer.c_custkey = orders.o_custkey where  customer.c_nationkey = 3 and customer.c_custkey < 500"
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
            ...
            queryId = 'TEST_08'
            query = "select AVG(CAST(COALESCE(n.n_nationkey, r.r_regionkey) AS DOUBLE)), MAX(COALESCE(n.n_nationkey, 8)) , COUNT(COALESCE(n.n_nationkey, 12)),  n.n_nationkey from nation as n left outer join region as r on n.n_nationkey = r.r_regionkey GROUP BY n.n_nationkey"
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
        ...
        #     query = "select SUM(COALESCE(n2.n_nationkey, 100)), COUNT(DISTINCT(COALESCE(n1.n_nationkey,32))), COALESCE(n2.n_regionkey, 100) as n1key from nation as n1 full outer join nation as n2 on n1.n_nationkey = n2.n_nationkey + 6 GROUP BY COALESCE(n2.n_regionkey, 100)"
            ...
            queryId = 'TEST_11'
            query = "select n.n_nationkey, COALESCE(r.r_comment, n.n_comment) from nation as n left outer join region as r on n.n_nationkey = r.r_regionkey"
            runTest.run_query(bc, drill, query, queryId, queryType, worder, '', acceptable_difference, use_percentage, fileSchemaType)
