...
if __name__ == '__main__':
    ...
    aggregration_queries = [
        ...
        'select count(p_partkey), sum(p_partkey), avg(p_partkey), max(p_partkey), min(p_partkey) from part where p_partkey < 100',
        # 'select count(p_partkey), sum(p_partkey), avg(p_partkey), max(p_partkey), min(p_partkey) from part where p_partkey < 0'
    ]
    ...
    groupby_queries = [
        ...
        'select count(c_custkey) + sum(c_acctbal) + avg(c_acctbal), min(c_custkey) - max(c_nationkey), c_nationkey * 2 as key from customer where  c_nationkey * 2 < 40 group by  c_nationkey * 2',
        'select c_nationkey, count(c_acctbal) from customer group by c_nationkey, c_custkey',
    ...
    orderby_queries = [
        ...
        'select c_custkey + c_nationkey, c_acctbal from customer order by 1, 2',
    ...
    join_queries = [
        ...
        # 'select n.n_nationkey, r.r_regionkey from nation as n left outer join region as r on n.n_nationkey = r.r_regionkey where n.n_nationkey < 10',
        # 'select n.n_nationkey, r.r_regionkey, n.n_nationkey + r.r_regionkey from nation as n left outer join region as r on n.n_nationkey = r.r_regionkey where n.n_nationkey < 10',
        'select n.n_nationkey, r.r_regionkey from nation as n left outer join region as r on n.n_regionkey = r.r_regionkey where n.n_nationkey < 10',
        # 'select n1.n_nationkey as n1key, n2.n_nationkey as n2key, n1.n_nationkey + n2.n_nationkey from nation as n1 full outer join nation as n2 on n1.n_nationkey = n2.n_nationkey + 6',
        # 'select n1.n_nationkey as n1key, n2.n_nationkey as n2key, n1.n_nationkey + n2.n_nationkey from nation as n1 full outer join nation as n2 on n1.n_nationkey = n2.n_nationkey + 6 where n1.n_nationkey < 10',
        'SELECT n.n_nationkey + 1, n.n_regionkey FROM nation AS n INNER JOIN region AS r ON n.n_regionkey = r.r_regionkey',
    ...
    # union_queries = [
    #     '(select o_orderkey, o_custkey from orders where o_orderkey < 100) union all (select o_orderkey, o_custkey from orders where o_orderkey < 300 and o_orderkey >= 200)',
    #     '(select o_orderkey, o_custkey from orders where o_orderkey < 100) union all (select o_orderkey, o_custkey from orders where o_orderkey < 300 and o_orderkey >= 200) order by 2',
    #     '(select o_orderkey, o_totalprice as key from orders where o_orderkey < 100) union all (select o_orderkey, o_custkey as keyy from orders where o_orderkey < 300 and o_orderkey >= 200)',
    # ]
