...
def main():
    ...
    # string_classes: str_tmp
    str_tmp = (make_unit_test(item, plan) for item, plan in zip(items, plans))
    header_text = "\n".join(str_tmp)
...
def make_unit_test(item, plan):
    return (
        ...
        "auto input_tables = ToBlazingFrame(input.filePaths,"
        " input.columnNames, input.columnTypes);"
...
def get_column_types(tables):
    ...
    def get_list(columnTypes):
        return "{%s}" % (",".join(['"%s"' % native_type(val) for val
                                   in columnTypes]))
