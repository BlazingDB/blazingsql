...
def make_plans(items, calcite_jar):
    def inputjson(item): return re.findall(
        'non optimized\\n(.*)\\n\\noptimized',
        subprocess.Popen(
            ('java',
             '-jar',
                    ...
                    'tables': [
                        {
                            ...
                            'dbName': item.schema.dbName}]}).encode())[0].decode('utf-8'),
        re.M | re.S)[0]
...
def Φ(item, plan):
    return (
        ...
        'tableGroup': make_table_group(
            item.data,
            item.schema.dbName + '.' + item.schema.tableName,
            item.schema.columnNames,
            item.schema.columnTypes),
