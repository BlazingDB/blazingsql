...
class BlazingConfig {
	public:
		...
		Uri getBaseFolder() const {
			return baseFolder;
		}

		Uri getRFolder() const {
			return rFolder;
		}

		Uri getPerfFolder() const {
					return perfFolder;
				}
		Uri getUploadFolder() const {
			return uploadFolder;
		}

		Uri getNodesConfigFileName() const {
			return nodesConfigFileName;
		}
		...
		bool getCheckFilePermissions() const {
			return checkFilePermissions;
		}

		unsigned long long getChunkDefaultSize() const {
			return CHUNK_DEFAULT_SIZE;
		}
		...
		int getDechacheingPeriod() const {
			return DECACHE_PERIOD;
		}

		int getMaxNumOfSimulteneousQueries() const {
			return MAX_NUM_SIMULTANEOUS_QUERIES;
		}

		unsigned long long getSimultaneousProcessingCapacity() const {
			return SIMULTANEOUS_PROCESSING_CAPACITY;
		}

		double getProcessingRequirmentsEstimationModifier() const {
			return PROCESSING_REQUIREMENT_ESTIMATION_MODIFIER;
		}

		double getInequalityJoinEstimationModifier() const {
			return INEQUALITY_JOIN_ESTIMATION_MODIFIER;
		}

		int getNumberOfImportBuffers() const {
			return NUMBER_IMPORT_BUFFERS;
		}

		int getVirtualNodeMultiplier() const {
			return VIRTUAL_NODE_MULTIPLIER;
		}
		void setVirtualNodeMultiplier(int numNodes){
			VIRTUAL_NODE_MULTIPLIER = numNodes;
		}

		void setFolders(std::string configFileName);

		void refresh(){
			setFolders(this->configFileName);
		}

		Uri getTempFolder() const{
			return this->tempFolder;
		}

		Uri getFSNamespacesFile() const{
			return this->fsNamespacesFile;
		}

		Uri getRandomSequentialFolder();

		std::vector<Uri> getSequentialFolders(){
			return this->sequentialFoldersList;
		}

		Uri getSequentialFoldersRoot(){
		...
		short getNumThreadsLoadDataParsingFile() const{
			return numThreadsLoadDataParsingFile;
		}

		short getNumThreadsForOptimization() const{
			return numThreadsForOptimization;
		}

		short getNumThreadsDecompressionLoading() const{
			return numThreadsDecompressionLoading;
		}

		short getNumThreadsDecompression() const{
			return numThreadsDecompression;
		}

		short getNumThreadsGetFilteredDataFromCache() const{
			return numThreadsGetFilteredDataFromCache;
		}

		short getNumThreadsCompressDataAndPersisit() const{
			return numThreadsCompressDataAndPersist;
		}

		void setNumThreadsCompressDataAndPersisit(short numThreads) {
			numThreadsCompressDataAndPersist = numThreads;
		}
		...
		short getNumThreadsForSkipDataGenLoad() const{
			return numThreadsForSkipDataGenLoad;
		}

		short getNumThreadsForSkipDataGenProcess() const{
			return numThreadsForSkipDataGenProcess;
		}
		...
		unsigned long long getMaxRowSize() const{
			return this->maxRowSize;
		}

		bool getCheckQueryPermissions() const {
			return checkQueryPermissions;
		}

		bool getDiskCacheEnabled() const {
			return diskCacheEnabled;
		}

		int getFileRetryDelay() const {
			return fileRetryDelay;
		}

		int getNumberOfSectionsAtATimeForSkipData() const{
			return numberOfSectionsAtATimeForSkipData;
		}

		unsigned long long getMaxNumberOfPartitionCombinations() const{
			return maxNumberOfPartitionCombinations;
		}

	short getHllPrecisionFactor() const {
		return hllPrecisionFactor;
	}

	short getNumThreadsLoadParquetFiles() const {
		return numThreadsLoadParquetFiles;
	}

	short getNumThreadsLoadingFromDiskCache() const {
	...
	int getNumStreamsTransformations() const {
		return numStreamsTransformations;
	}
	unsigned long long getDiskCacheSize(){
		return this->diskCacheSize;
	}

	Uri getDiskCacheLocation(){
	...
	private:
		...
		unsigned long long SIMULTANEOUS_PROCESSING_CAPACITY; // in bytes. How much operating memory it has to process queries simultaneously. Currently based entirely off of heuristic estimates
		...
		double TRANSFORMATION_PROCESSOR_OVERHEAD; // Range [0,1] this much memory overhead will use the transformation processor by maxBatchsize
...
static int NULL_INT = (INT_MAX - 1)/2 + 1;  //1073741824;   // our valid range is -1073741823 to 1073741823 which is half of what c++ allows, but we need to be able to do delta compression on it
...
template <> float getNullValue<float>();
template <> int getNullValue<int>();
template <> short getNullValue<short>();
template <> char getNullValue<char>();
