...
float getMinPossibleValueForDataType(float value)
{
	return FLT_MIN;
}
...
void BlazingConfig::setFolders(std::string configFileName){
	...
	std::shared_ptr<arrow::io::RandomAccessFile> file = BlazingContext::getInstance()->getFileSystemManager()->openReadable(configFileNameUri);
