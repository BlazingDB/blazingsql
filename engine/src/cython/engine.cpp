...
void make_sure_output_is_not_input_gdf(blazing_frame & output_frame, std::vector<TableSchema> & tableSchemas, const std::vector<int> & fileTypes){
  ...
  if (input_data_ptrs.size() > 0){
    for(size_t index = 0; index < output_frame.get_width(); index++){
      ...
      if (std::find(input_data_ptrs.begin(), input_data_ptrs.end(), output_frame.get_column(index).data()) != input_data_ptrs.end()){
...
ResultSet runQuery(int32_t masterIndex, std::vector<NodeMetaDataTCP> tcpMetadata,
   std::vector<std::string> tableNames, std::vector<TableSchema> tableSchemas,
   std::vector<std::vector<std::string>> tableSchemaCppArgKeys, std::vector<std::vector<std::string>> tableSchemaCppArgValues,
   std::vector<std::vector<std::string>> filesAll, std::vector<int> fileTypes,
    int32_t ctxToken, std::string query, uint64_t accessToken,
  ...
  for(int i = 0; i < tableSchemas.size(); i++){
    ...
    auto schema = ral::io::Schema(tableSchema.names, tableSchema.calcite_to_file_indices,
        types, tableSchema.num_row_groups,time_units,tableSchema.in_file);
    ...
    if(fileType == ral::io::DataType::CUDF || fileType == ral::io::DataType::DASK_CUDF){
      ...
      provider = std::make_shared<ral::io::uri_data_provider>(uris,uri_values[i],string_values[i],is_column_string[i]);
    }
  ...
  try {
    ...
    for(auto currentMetadata: tcpMetadata){
      ...
      auto address = blazingdb::transport::Address::TCP(currentMetadata.ip, currentMetadata.communication_port, 0);
