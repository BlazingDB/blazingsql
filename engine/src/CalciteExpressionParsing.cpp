...
bool is_exponential_operator(gdf_binary_operator_exp operation){
	return operation == BLZ_POW;
}
...
bool is_logical_operation(gdf_binary_operator_exp operation){
	return (operation == BLZ_EQUAL ||
					operation == BLZ_NOT_EQUAL ||
					operation == BLZ_GREATER||
					operation == BLZ_GREATER_EQUAL ||
					operation == BLZ_LESS ||
					operation == BLZ_LESS_EQUAL ||
...
gdf_dtype get_output_type(gdf_dtype input_left_type, gdf_dtype input_right_type, gdf_binary_operator_exp operation){
	...
	if(is_arithmetic_operation(operation)){
		...
		if(is_type_float(input_left_type) || is_type_float(input_right_type) ){
			...
			if(is_type_float(input_left_type) && is_type_float(input_right_type) ){
				return (ral::traits::get_dtype_size_in_bytes(input_left_type) >= ral::traits::get_dtype_size_in_bytes(input_right_type)) ? input_left_type : input_right_type;
		...
		if(is_type_signed(input_left_type)){
			//left signed
			//right unsigned
			if(ral::traits::get_dtype_size_in_bytes(input_left_type) > ral::traits::get_dtype_size_in_bytes(input_right_type)){
...
void get_common_type(gdf_dtype type1, gdf_dtype_extra_info info1, gdf_dtype type2, gdf_dtype_extra_info info2, gdf_dtype & type_out, gdf_dtype_extra_info & info_out){
	...
	if (type1 == type2){
		...
		type_out = (ral::traits::get_dtype_size_in_bytes(type1) >= ral::traits::get_dtype_size_in_bytes(type2)) ? type1 : type2;
	...
	} else if ((type1 == GDF_STRING || type1 == GDF_STRING_CATEGORY) && (type2 == GDF_STRING || type2 == GDF_STRING_CATEGORY)){
...
gdf_dtype infer_dtype_from_literal(const std::string& token) {
	if (is_null(token)){
		...
		if (token.find_first_of(".eE") != std::string::npos) {
			...
			return parsed_double == casted_float
						? GDF_FLOAT32
						: GDF_FLOAT64;
		} else {
			int64_t parsed_int64 = std::stoll(token);
			return parsed_int64 > std::numeric_limits<int32_t>::max() || parsed_int64 < std::numeric_limits<int32_t>::min()
						? GDF_INT64 
						: parsed_int64 > std::numeric_limits<int16_t>::max() || parsed_int64 < std::numeric_limits<int16_t>::min()
						? GDF_INT32 
						: parsed_int64 > std::numeric_limits<int8_t>::max() || parsed_int64 < std::numeric_limits<int8_t>::min()
						? GDF_INT16
						: GDF_INT8;
		}
	...
	for (std::string token : tokens) {
		...
		if(is_operator_token(token)){
			if(is_binary_operator_token(token) ){
				...
				if(ral::traits::get_dtype_size_in_bytes(operands.top()) > ral::traits::get_dtype_size_in_bytes(*max_temp_type)){
...
void fix_tokens_after_call_get_tokens_in_reverse_order_for_timestamp(blazing_frame & inputs, std::vector<std::string> &tokens) {
    ...
    if (has_timestamp) {
        ...
        if (coms) {
            for (int i = 0; i < tokens.size(); ++i) {
                ...
                if (!StringUtil::contains(tok, "'")) continue;
...
get_index(std::string operand_string) {
    if (operand_string.empty()) { return 0; }
...
std::string replace_calcite_regex(std::string expression){
	...
	static const std::regex char_collate_re{R""((?:\(\d+\))? CHARACTER SET ".+?" COLLATE ".+?")"", std::regex_constants::icase};
	...
	static const std::regex number_implicit_cast_re{R""((\d):(?:DECIMAL\(\d+, \d+\)|INTEGER|BIGINT|FLOAT|DOUBLE))"", std::regex_constants::icase};
	expression = std::regex_replace(expression, number_implicit_cast_re, "$1");

	static const std::regex null_implicit_cast_re{R""(null:(?:DECIMAL\(\d+, \d+\)|INTEGER|BIGINT|FLOAT|DOUBLE))"", std::regex_constants::icase};
