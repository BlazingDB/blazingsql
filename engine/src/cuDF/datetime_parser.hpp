/**
 * Copied from cudf: cpp/src/io/csv/datetime_parser.cuh
 * All functions were converted from __device__ functions to normal functions
 */

#ifndef DATETIME_PARSER_HPP_
#define DATETIME_PARSER_HPP_
...
inline bool extractDate(const char * data, long sIdx, long eIdx, bool dayfirst, int * year, int * month, int * day);
inline void extractTime(
	const char * data, long start, long end, int * hour, int * minute, int * second, int * millisecond);

inline constexpr int32_t daysSinceEpoch(int year, int month, int day);
inline constexpr int64_t secondsSinceEpoch(int year, int month, int day, int hour, int minute, int second);
 ...
 *---------------------------------------------------------------------------**/
template <typename T>
inline T convertStrToInteger(const char * data, long start, long end) {
	T value = 0;
...
inline gdf_date32 parseDateFormat(const char * data, long start_idx, long end_idx, bool dayfirst) {
	int day, month, year;
	gdf_date32 e = -1;
 ...
 */
inline gdf_date64 parseDateTimeFormat(const char * data, long start, long end, bool dayfirst) {
	int day, month, year;
...
inline constexpr uint32_t operator"" _days(unsigned long long int days) { return days; }

inline constexpr uint32_t operator"" _erasInDays(unsigned long long int eras) {
	// multiply by number of days within an era (400 year span)
	return eras * 146097_days;
}

inline constexpr uint32_t operator"" _years(unsigned long long int years) { return years; }

inline constexpr uint32_t operator"" _erasInYears(unsigned long long int eras) {
 ...
 */
inline constexpr int32_t daysSinceBaseline(int year, int month, int day) {
...
inline constexpr int32_t daysSinceEpoch(int year, int month, int day) {
	...
	static_assert(daysSinceBaseline(1970, 1, 1) == 719468_days, "Baseline to epoch returns incorrect number of days");
