...
std::string blazing_table_view_schema_to_string(ral::frame::BlazingTableView table_view, const std::string table_name){
	...
	for(size_t col_idx=0; col_idx<table_view.num_columns(); col_idx++){
		ostream<<"\t"<<table_view.names().at(col_idx)<<" ("<<"type: "<<type_string(table_view.column(col_idx).type())<<") - isnullable: "<< table_view.column(col_idx).nullable() << " | null_count(): " << table_view.column(col_idx).null_count() << std::endl;
	}
	return ostream.str();
}
