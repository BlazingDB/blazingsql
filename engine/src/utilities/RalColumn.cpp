...
gdf_column_cpp create_column(const gdf_size_type size, const gdf_dtype dtype, const std::string name) {
    ...
    extra_info.time_unit = (dtype == GDF_DATE64 || dtype == GDF_TIMESTAMP ? TIME_UNIT_ms : TIME_UNIT_NONE); // TODO this should not be hardcoded
...
cudf::table create_table(const std::vector<gdf_column_cpp> & columns){
	...
	std::transform(columns.begin(), columns.end(), 
						columns_ptrs.begin(), [&](const gdf_column_cpp& el) {
