...
std::vector<gdf_column_cpp> concatTables(const std::vector<std::vector<gdf_column_cpp>>& tables){
    ...
    if (tables[i].size() > 0) {
      ...
      if (num_columns == 0){
        ...
        if (table.size() != num_columns){
          Library::Logging::Logger().logError(buildLogString("", "", "", "ERROR: tables being concatenated are not the same size"));
        }
  ...
  for(size_t i = 0; i < num_columns; i++) {
    ...
    if (std::any_of(raw_columns_to_concat.cbegin(), raw_columns_to_concat.cend(), [](const gdf_column* c){ return c->valid != nullptr; })) {
      output_table[i].create_gdf_column(raw_columns_to_concat[0]->dtype, raw_columns_to_concat[0]->dtype_info, output_row_size, nullptr, ral::traits::get_dtype_size_in_bytes(raw_columns_to_concat[0]->dtype), std::string(raw_columns_to_concat[0]->col_name));
    } else {
      output_table[i].create_gdf_column(raw_columns_to_concat[0]->dtype, raw_columns_to_concat[0]->dtype_info, output_row_size, nullptr, nullptr, ral::traits::get_dtype_size_in_bytes(raw_columns_to_concat[0]->dtype), std::string(raw_columns_to_concat[0]->col_name));
    }

  
    CUDF_CALL( gdf_column_concat(output_table[i].get_gdf_column(),
                                raw_columns_to_concat.data(),
                                raw_columns_to_concat.size()) );
...
std::vector<gdf_column_cpp> normalizeColumnTypes(std::vector<gdf_column_cpp> columns) {
  ...
  for(size_t j = 1; j < columns.size(); j++) {
    ...
    if (type_out == GDF_invalid) {
      throw std::runtime_error("In normalizeColumnTypes function: no common type between " + std::to_string(common_type) + " and " + std::to_string(columns[j].dtype()));
    }
  ...
  for(size_t j = 0; j < columns.size(); j++) {
    if (common_type == GDF_TIMESTAMP){
      if (columns[j].dtype() == common_type && columns[j].dtype_info().time_unit == common_info.time_unit) {
        ...
        Library::Logging::Logger().logWarn(buildLogString("", "", "", "WARNING: normalizeColumnTypes casting " + std::to_string(columns[j].get_gdf_column()->dtype) + " to " + std::to_string(common_type)));
