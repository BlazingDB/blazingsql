...
gdf_column_cpp handle_cast_from_string(gdf_unary_operator operation, cudf::column * input_col) {
...
gdf_column_cpp handle_cast_to_string(cudf::column * input_col) {
...
int insert_string_into_column_nvcategory(cudf::column * col, const std::string & str) {
	// TODO percy cudf0.12 port to cudf::column and custrings
...
gdf_column_cpp handle_match_regex(cudf::column * input_col, const std::string & re) {
	// TODO percy cudf0.12 custrings this was not commented
//	NVCategory * nv_category = static_cast<NVCategory *>(input_col->dtype_info.category);
//	NVStrings * nv_strings =
//		nv_category->gather_strings(static_cast<nv_category_index_type *>(input_col->data), input_col->size);
...
gdf_column_cpp handle_substring(cudf::column * input_col, const std::string & str_params) {
	// TODO percy cudf0.12 custrings this was not commented
...
gdf_column_cpp handle_concat_str_literal(cudf::column * input_col, const std::string & str, bool prefix = false) {
	// TODO percy cudf0.12 custrings this was not commented
...
gdf_column_cpp handle_concat_str_col(cudf::column * left_input_col, cudf::column * right_input_col) {
	// TODO percy cudf0.12 port to cudf::column and custrings
...
//					right_inputs.push_back(right.is_valid ? SCALAR_INDEX : SCALAR_NULL_INDEX);
					...
					cudf::column * left_column = input_columns[mapped_index];
...
//					output_column->dtype_info.category = static_cast<NVCategory *>(src_col->dtype_info.category)->copy();
			...
			if(is_literal(token) || is_string(token)) {
				...
				src_str_col_map[mapped_idx] = (input_columns[mapped_idx]->type().id() == cudf::type_id::STRING ? mapped_idx : -1);
			}
		}
	}
}
...
void evaluate_expression(blazing_frame & inputs, const std::string & expression, gdf_column_cpp & output) {
	...
	std::vector<cudf::column *> output_columns(1);
	...
	output_type_expressions[0] = output.get_gdf_column()->type().id();
