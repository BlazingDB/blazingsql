...
gdf_column_cpp handle_cast_from_string(gdf_unary_operator operation, gdf_column* input_col){
	...
	NVStrings* nv_strings = nv_category->gather_strings(static_cast<nv_category_index_type*>(input_col->data), input_col->size);
	...
	extra_info.time_unit = (cast_type == GDF_TIMESTAMP ? TIME_UNIT_ms : TIME_UNIT_NONE); // TODO this should not be hardcoded

	gdf_column_cpp new_input_col;
	new_input_col.create_gdf_column(cast_type, extra_info, input_col->size, nullptr, ral::traits::get_dtype_size_in_bytes(cast_type));

	switch (cast_type)
	{
	case GDF_INT32:
		nv_strings->stoi(static_cast<int*>(new_input_col.data()));
		break;
	case GDF_INT64:
		nv_strings->stol(static_cast<long*>(new_input_col.data()));
		break;
	case GDF_FLOAT32:
		nv_strings->stof(static_cast<float*>(new_input_col.data()));
		break;
	case GDF_FLOAT64:
		nv_strings->stod(static_cast<double*>(new_input_col.data()));
		break;
	case GDF_DATE32:
	...
	case GDF_TIMESTAMP:
		...
		nv_strings->timestamp2long("%Y-%m-%dT%H:%M:%SZ", NVStrings::ms, static_cast<unsigned long*>(new_input_col.data()));
	...
	if (input_col->null_count) {
		...
		CheckCudaErrors(cudaMemcpy(new_input_col.valid(), input_col->valid, gdf_valid_allocation_size(new_input_col.size()), cudaMemcpyDefault));
...
gdf_column_cpp handle_cast_to_string(gdf_column* input_col){
	...
	if(input_col->size > 0){
		...
		case GDF_DATE32:
		case GDF_DATE64:
			nv_strings = NVStrings::long2timestamp(static_cast<unsigned long*>(input_col->data), input_col->size, NVStrings::days, "%Y-%m-%d", input_col->valid);
			break;
		case GDF_TIMESTAMP:
			if (input_col->dtype_info.time_unit == TIME_UNIT_ns) {
				nv_strings = NVStrings::long2timestamp(static_cast<unsigned long*>(input_col->data), input_col->size, NVStrings::ns, "%Y-%m-%dT%H:%M:%SZ", input_col->valid);
			}
			else {
				nv_strings = NVStrings::long2timestamp(static_cast<unsigned long*>(input_col->data), input_col->size, NVStrings::ms, "%Y-%m-%dT%H:%M:%SZ", input_col->valid);
			}
			break;
		default:
			assert(false);
	...
	NVCategory* nv_category = nv_strings ? NVCategory::create_from_strings(*nv_strings) : NVCategory::create_from_array(nullptr, 0);
	NVStrings::destroy(nv_strings);
...
int insert_string_into_column_nvcategory(gdf_column* col, const std::string& str){
	...
	CheckCudaErrors(cudaMemcpyAsync(col->data,
																	new_category->values_cptr(),
																	sizeof(gdf_nvstring_category) * col->size,
...
gdf_column_cpp handle_match_regex(gdf_column* input_col, const std::string& re){
	...
	gdf_column_cpp new_input_col;
	new_input_col.create_gdf_column(GDF_BOOL8, gdf_dtype_extra_info{TIME_UNIT_NONE, nullptr}, input_col->size, nullptr, nullptr, ral::traits::get_dtype_size_in_bytes(GDF_BOOL8));
...
gdf_column_cpp handle_concat_str_col(gdf_column* left_input_col, gdf_column* right_input_col){
	...
	NVStrings* left_nv_strings = left_nv_category->gather_strings(static_cast<nv_category_index_type*>(left_input_col->data), left_input_col->size);

	NVCategory* right_nv_category = static_cast<NVCategory *>(right_input_col->dtype_info.category);
	NVStrings* right_nv_strings = right_nv_category->gather_strings(static_cast<nv_category_index_type*>(right_input_col->data), right_input_col->size);
		...
		if(is_operator_token(token)){
			...
			if(is_binary_operator_token(token)){
				...
				if(is_literal(left_operand) && is_literal(right_operand)){
                    ...
                    gdf_scalar left = get_scalar_from_string(left_operand,get_type_from_string(left_operand), extra_info);
					left_scalars.push_back(left);
                    ...
                    gdf_scalar right = get_scalar_from_string(right_operand,get_type_from_string(right_operand), extra_info);
					...
					std::string literal_operand = is_string(left_operand) ? left_operand.substr(1,left_operand.size()-2) : right_operand.substr(1,right_operand.size()-2);
					...
					if(left_column != nullptr){
						if (operation == BLZ_STR_LIKE) {
							...
							gdf_column_cpp new_input_col = handle_concat_str_literal(left_column, literal_operand, is_string(left_operand));
							...
							int idx_position = static_cast<NVCategory *>(left_column->dtype_info.category)->get_value(literal_operand.c_str());
							if(idx_position == -1){
				...
				if(is_literal(left_operand)){
					...
					} else if(operation == BLZ_CAST_INTEGER
										|| operation == BLZ_CAST_BIGINT
										|| operation == BLZ_CAST_FLOAT
										|| operation == BLZ_CAST_DOUBLE
										|| operation == BLZ_CAST_DATE
										|| operation == BLZ_CAST_TIMESTAMP) {
						if (mapped_left_index >= 0 && input_columns[mapped_left_index] != nullptr && input_columns[mapped_left_index]->size > 0) {
							gdf_column* left_column = input_columns[mapped_left_index];
			...
			if(is_literal(token) || is_string(token)){
				...
				src_str_col_map[mapped_idx] = (input_columns[mapped_idx]->dtype == GDF_STRING_CATEGORY ? mapped_idx : -1);
			}
		}
	}
