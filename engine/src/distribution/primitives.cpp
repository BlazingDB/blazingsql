...
void sendSamplesToMaster(const Context& context, std::vector<gdf_column_cpp>& samples, std::size_t total_row_size) {
    ...
    auto message = MessageFactory::createSampleToNodeMaster(message_id,
                                                            context_token,
                                                            self_node,
                                                            total_row_size,
                                                            samples);
    ...
    Library::Logging::Logger().logTrace(ral::utilities::buildLogString(std::to_string(context_token), std::to_string(context.getQueryStep()), std::to_string(context.getQuerySubstep()), "About to send sendSamplesToMaster message"));
...
std::vector<NodeSamples> collectSamples(const Context& context) {
  ...
  for (int k = 0; k < size; ++k) {
    ...
    if (received[node_idx]){
      Library::Logging::Logger().logError(ral::utilities::buildLogString(std::to_string(context_token), std::to_string(context.getQueryStep()), std::to_string(context.getQuerySubstep()), "ERROR: Already received collectSamples from node " + std::to_string(node_idx)));
    } 
...
std::vector<gdf_column_cpp> generatePartitionPlans(const Context& context, std::vector<NodeSamples>& samples, std::vector<int8_t>& sortOrderTypes){
  ...
  std::transform(concatSamples.cbegin(), concatSamples.cend(), rawConcatSamples.begin(), [](const gdf_column_cpp& col){
    return col.get_gdf_column();
  });

  // Sort
  gdf_column_cpp ascDescCol;
	ascDescCol.create_gdf_column(GDF_INT8, gdf_dtype_extra_info{TIME_UNIT_NONE,nullptr}, sortOrderTypes.size(), sortOrderTypes.data(), ral::traits::get_dtype_size_in_bytes(GDF_INT8), "");

  gdf_column_cpp sortedIndexCol;
	sortedIndexCol.create_gdf_column(GDF_INT32, gdf_dtype_extra_info{TIME_UNIT_NONE,nullptr}, outputRowSize, nullptr, ral::traits::get_dtype_size_in_bytes(GDF_INT32), "");
  ...
  std::vector<gdf_column_cpp> sortedSamples(concatSamples.size());
    ...
    if (col.valid()) {
      sortedSamples[i].create_gdf_column(col.dtype(), col.dtype_info(), col.size(), nullptr, ral::traits::get_dtype_size_in_bytes(col.dtype()), col.name());
    } else {
      sortedSamples[i].create_gdf_column(col.dtype(), col.dtype_info(), col.size(), nullptr, nullptr, ral::traits::get_dtype_size_in_bytes(col.dtype()), col.name());
    }
  ...
  std::vector<gdf_column_cpp> pivots(sortedSamples.size());
    ...
    if (col.valid()) {
      pivots[i].create_gdf_column(col.dtype(), col.dtype_info(), pivotsSize, nullptr, ral::traits::get_dtype_size_in_bytes(col.dtype()), col.name());
    } else {
      pivots[i].create_gdf_column(col.dtype(), col.dtype_info(), pivotsSize, nullptr, nullptr, ral::traits::get_dtype_size_in_bytes(col.dtype()), col.name());
    }
	}
  ...
  if (outputRowSize > 0 ){
    ...
    gatherMap.create_gdf_column(GDF_INT32, gdf_dtype_extra_info{TIME_UNIT_NONE,nullptr}, context.getTotalNodes() - 1, nullptr, ral::traits::get_dtype_size_in_bytes(GDF_INT32), "");
...
void distributePartitionPlan(const Context& context, std::vector<gdf_column_cpp>& pivots){
  ...
  auto message = Factory::createPartitionPivotsMessage(message_id,
                                                      context_token,
                                                      node,
                                                      pivots);
...
std::vector<NodeColumns> partitionData(const Context& context,
    ...
    if (!isTableSorted) {
      ...
      std::transform(searchColIndices.cbegin(), searchColIndices.cend(), key_cols_vect.begin(),
                  [&] (const int index) { return table[index].get_gdf_column(); });

      gdf_column_cpp asc_desc_col;
      asc_desc_col.create_gdf_column(GDF_INT8, gdf_dtype_extra_info{TIME_UNIT_NONE,nullptr}, sortOrderTypes.size(), sortOrderTypes.data(), ral::traits::get_dtype_size_in_bytes(GDF_INT8), "");

      gdf_column_cpp index_col;
      index_col.create_gdf_column(GDF_INT32, gdf_dtype_extra_info{TIME_UNIT_NONE,nullptr}, table_row_size, nullptr, ral::traits::get_dtype_size_in_bytes(GDF_INT32), "");
      ...
      for(size_t i = 0; i < sortedTable.size(); i++) {
        auto& col = table[i];
        if (col.valid()) {
          sortedTable[i].create_gdf_column(col.dtype(), col.dtype_info(), col.size(), nullptr, ral::traits::get_dtype_size_in_bytes(col.dtype()), col.name());
        } else {
          sortedTable[i].create_gdf_column(col.dtype(), col.dtype_info(), col.size(), nullptr, nullptr, ral::traits::get_dtype_size_in_bytes(col.dtype()), col.name());
        }
    ...
    std::transform(searchColIndices.cbegin(), searchColIndices.cend(), sync_haystack.begin(),
                  [&] (const int index) { return table[index]; });
    ...
    for (gdf_size_type i = 0; i < sync_haystack.size(); i++) {
        ...
        if (left_col.valid()) {
            CUDA_TRY( cudaMemcpy(new_left_column_ptr->valid, left_col.valid(), gdf_valid_allocation_size(new_left_column_ptr->size), cudaMemcpyDefault) );
            new_left_column_ptr->null_count = left_col.null_count();
        }

        if (right_col.valid()) {
            CUDA_TRY( cudaMemcpy(new_right_column_ptr->valid, right_col.valid(), gdf_valid_allocation_size(new_right_column_ptr->size), cudaMemcpyDefault) );
...
void distributePartitions(const Context& context, std::vector<NodeColumns>& partitions){
  ...
  for(auto& nodeColumn : partitions)
  {
    ...
    threads.push_back(std::thread([message_id, context_token, self_node, destination_node, columns]() mutable {
      ...
      auto message = Factory::createColumnDataMessage(message_id,
                                                      context_token,
                                                      self_node,
                                                      columns);
...
std::vector<NodeColumns> collectSomePartitions(const Context& context, int num_partitions) {
    ...
    while (0 < num_partitions) {
        ...
        int node_idx = context.getNodeIndex(*node);
        if (received[node_idx]){
          Library::Logging::Logger().logError(ral::utilities::buildLogString(std::to_string(context_token), std::to_string(context.getQueryStep()), std::to_string(context.getQuerySubstep()), "ERROR: Already received collectSomePartitions from node " + std::to_string(node_idx)));
        } 
...
std::vector<gdf_column_cpp> generatePartitionPlansGroupBy(const Context& context, std::vector<NodeSamples>& samples){
  ...
  gdf_column_cpp sortedIndexCol;
  sortedIndexCol.create_gdf_column(GDF_INT32, gdf_dtype_extra_info{TIME_UNIT_NONE,nullptr}, number_of_groups, nullptr, ral::traits::get_dtype_size_in_bytes(GDF_INT32), "");
...
void distributeRowSize(const Context& context, std::size_t total_row_size) {
  ...
  auto self_node = CommunicationData::getInstance().getSharedSelfNode();
  auto message = Factory::createSampleToNodeMaster(message_id,
                                                    context_token,
                                                    self_node,
                                                    total_row_size,
                                                    {});
...
std::vector<gdf_size_type> collectRowSize(const Context& context) {
  ...
  for(gdf_size_type i = 0; i < num_nodes-1; ++i) {
    ...
    assert(node_idx >= 0);
    if (received[node_idx]){
      Library::Logging::Logger().logError(ral::utilities::buildLogString(std::to_string(context_token), std::to_string(context.getQueryStep()), std::to_string(context.getQuerySubstep()), "ERROR: Already received collectRowSize from node " + std::to_string(node_idx)));
    }
...
void distributeLeftRightNumRows(const Context& context, std::size_t left_num_rows, std::size_t right_num_rows) {
  ...
  num_rows[0].create_gdf_column(GDF_INT64, gdf_dtype_extra_info{}, 2, &num_rows_host[0], ral::traits::get_dtype_size_in_bytes(GDF_INT64), "");
  auto message = Factory::createSampleToNodeMaster(message_id,
                                                    context_token,
                                                    self_node,
                                                    0,
...
void collectLeftRightNumRows(const Context& context, std::vector<gdf_size_type> & node_num_rows_left, std::vector<gdf_size_type> & node_num_rows_right) {
  ...
  for(gdf_size_type i = 0; i < num_nodes-1; ++i) {
    ...
    cudaMemcpy(num_rows_host.data(), num_rows_data[0].data(), ral::traits::get_dtype_size_in_bytes(GDF_INT64)*2, cudaMemcpyDeviceToHost);
    int node_idx = context.getNodeIndex(*node);
    assert(node_idx >= 0);
    if (received[node_idx]){
      Library::Logging::Logger().logError(ral::utilities::buildLogString(std::to_string(context_token), std::to_string(context.getQueryStep()), std::to_string(context.getQuerySubstep()), "ERROR: Already received collectLeftRightNumRows from node " + std::to_string(node_idx)));
    }
...
std::vector<NodeColumns> generateJoinPartitions(const Context& context,
    ...
    for (size_t i = 0; i < columnIndices.size(); i++) {
      ...
      str_hash.create_gdf_column(GDF_INT32, gdf_dtype_extra_info{TIME_UNIT_NONE,nullptr}, nvStrings->size(), nullptr, nullptr, ral::traits::get_dtype_size_in_bytes(GDF_INT32), "");
    ...
    for (size_t i = 0; i < output_columns.size(); i++) {
      if (output_columns[i].dtype() == GDF_STRING_CATEGORY && temp_input_table[i].dtype_info().category){
        output_columns[i].get_gdf_column()->dtype_info.category =  static_cast<void*>(static_cast<NVCategory *>(temp_input_table[i].dtype_info().category)->copy());
      }
    }
    ...
    gdf_column_cpp indexes;
    indexes.create_gdf_column(GDF_INT32, gdf_dtype_extra_info{TIME_UNIT_NONE,nullptr}, number_nodes - 1, partition_offset.data() + 1, nullptr, ral::traits::get_dtype_size_in_bytes(GDF_INT32), "");
