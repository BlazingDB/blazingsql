...
void parseJoinConditionToColumnIndices(const std::string& condition, std::vector<int>& columnIndices) {
    ...
    // NOTE percy c.cordoba here we dont need to call fix_tokens_after_call_get_tokens_in_reverse_order_for_timestamp after
...
void evaluate_join(std::string condition,
	...
	if(join_type == INNER_JOIN){
		...
		cudf::table result = cudf::full_join(
														cudf::table{left_columns},
														cudf::table{right_columns},
														join_cols,
														join_cols,
