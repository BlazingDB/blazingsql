...
void result_set_repository::update_token(
	...
	for(size_t i = 0; i < frame.get_width(); i++) {
		if(frame.get_column(i).get_gdf_column()->type().id() == cudf::type_id::STRING) {
			...
			if(frame.get_column(i).get_gdf_column()->size() > 0) {
...
//				NVCategory::destroy(new_category);
			...
			string_column.create_gdf_column(new_strings, frame.get_column(i).get_gdf_column()->size(), frame.get_column(i).name());
...
gdf_column_cpp result_set_repository::get_column(connection_id_t connection, column_token_t columnToken) {
	...
	if(this->precalculated_columns[columnToken].get_gdf_column()->type().id() == cudf::STRING) {
