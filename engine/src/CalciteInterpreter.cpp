...
project_plan_params parse_project_plan(blazing_frame& input, std::string query_part) {
	...
	for(int i = 0; i < expressions.size(); i++){ //last not an expression
		...
		if(contains_evaluation(expression)){
			...
			extra_info.time_unit = (output_type_expressions[i] == GDF_TIMESTAMP ? TIME_UNIT_ms : TIME_UNIT_NONE); // TODO this should not be hardcoded

			//assumes worst possible case allocation for output
			//TODO: find a way to know what our output size will be
			gdf_column_cpp output;
			output.create_gdf_column(output_type_expressions[i],extra_info,size,nullptr,ral::traits::get_dtype_size_in_bytes(output_type_expressions[i]), name);
			...
			if (is_literal_col) {
				...
				if(col_type == GDF_STRING_CATEGORY){
					...
					gdf_dtype_extra_info extra_info;
					extra_info.category = nullptr;
					extra_info.time_unit = (col_type == GDF_DATE64 || col_type == GDF_TIMESTAMP ? TIME_UNIT_ms : TIME_UNIT_NONE); // TODO this should not be hardcoded
...
void process_filter(Context * context, blazing_frame & input, std::string query_part){
	...
	stencil.create_gdf_column(GDF_BOOL8,gdf_dtype_extra_info{TIME_UNIT_NONE, nullptr},input.get_num_rows_in_table(0),nullptr,ral::traits::get_dtype_size_in_bytes(GDF_BOOL8), "");

	Library::Logging::Logger().logInfo(timer.logDuration(*context, "Filter part 1 initialize stencil", "num rows", input.get_num_rows_in_table(0)));
	timer.reset();
	...
	Library::Logging::Logger().logInfo(timer.logDuration(*context, "Filter part 2 evaluate expression", "num rows", input.get_num_rows_in_table(0)));
	timer.reset();

	gdf_column_cpp index_col;
	std::string empty = "";
	index_col.create_gdf_column(GDF_INT32,gdf_dtype_extra_info{TIME_UNIT_NONE, nullptr},input.get_num_rows_in_table(0),nullptr,ral::traits::get_dtype_size_in_bytes(GDF_INT32), empty);
	...
	Library::Logging::Logger().logInfo(timer.logDuration(*context, "Filter part 3 apply_boolean_mask", "num rows", input.get_num_rows_in_table(0)));
	timer.reset();
}
...
blazing_frame evaluate_split_query(
	...
	if(is_double_input(query[0])){
		...
		if (ral::operators::is_join(query[0])) {
			...
			std::string extraInfo = "left_side_num_rows:" + std::to_string(numLeft) + ":right_side_num_rows:" + std::to_string(numRight);
			Library::Logging::Logger().logInfo(blazing_timer.logDuration(*queryContext, "evaluate_split_query process_union", "num rows result", result_frame.get_num_rows_in_table(0), extraInfo));			
			blazing_timer.reset();
			return result_frame;
		}else{
		...
		if(is_project(query[0])){
			...
			Library::Logging::Logger().logInfo(blazing_timer.logDuration(*queryContext, "evaluate_split_query process_project", "num rows", child_frame.get_num_rows_in_table(0)));			
			blazing_timer.reset();
			return child_frame;
		}else if(ral::operators::is_aggregate(query[0])){
			blazing_timer.reset();  // doing a reset before to not include other calls to evaluate_split_query
			ral::operators::process_aggregate(child_frame, query[0], queryContext);
			Library::Logging::Logger().logInfo(blazing_timer.logDuration(*queryContext, "evaluate_split_query process_aggregate", "num rows", child_frame.get_num_rows_in_table(0)));			
			blazing_timer.reset();
			return child_frame;
		}else if(ral::operators::is_sort(query[0])){
			blazing_timer.reset();  // doing a reset before to not include other calls to evaluate_split_query
			ral::operators::process_sort(child_frame, query[0], queryContext);
			Library::Logging::Logger().logInfo(blazing_timer.logDuration(*queryContext, "evaluate_split_query process_sort", "num rows", child_frame.get_num_rows_in_table(0)));						
			blazing_timer.reset();
			return child_frame;
		}else if(is_filter(query[0])){
			blazing_timer.reset();  // doing a reset before to not include other calls to evaluate_split_query
			process_filter(queryContext, child_frame,query[0]);
			Library::Logging::Logger().logInfo(blazing_timer.logDuration(*queryContext, "evaluate_split_query process_filter", "num rows", child_frame.get_num_rows_in_table(0)));			
			blazing_timer.reset();
			return child_frame;
		}else{
			throw std::runtime_error{"In evaluate_split_query function: unsupported query operator"};
	...
	if(query.size() == 1){
		...
		if(is_scan(query[0])){
			...
			if(is_bindable_scan(query[0])){
				...
				std::vector<std::string> aliases_string_split = get_expressions_from_expression_list(aliases_string, true);
				...
				for(size_t col_idx=0; col_idx < aliases_string_split.size(); col_idx++){
					//TODO: Rommel, this check is needed when for example the scan has not projects but there are extra aliases
				...
				if(is_filtered_bindable_scan(query[0])){
					...
					Library::Logging::Logger().logInfo(blazing_timer.logDuration(*queryContext, "evaluate_split_query process_filter", "num rows", scan_frame.get_num_rows_in_table(0)));								
					blazing_timer.reset();
