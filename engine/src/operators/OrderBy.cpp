...
void sort(const Context & queryContext, blazing_frame& input, std::vector<gdf_column*>& rawCols, std::vector<int8_t>& sortOrderTypes, std::vector<gdf_column_cpp>& sortedTable){
	...
	index_col.create_gdf_column(GDF_INT32, gdf_dtype_extra_info{TIME_UNIT_NONE, nullptr}, input.get_num_rows_in_table(0), nullptr, ral::traits::get_dtype_size_in_bytes(GDF_INT32), "");
...
void single_node_sort(const Context & queryContext, blazing_frame& input, std::vector<gdf_column*>& rawCols, std::vector<int8_t>& sortOrderTypes) {
	std::vector<gdf_column_cpp> sortedTable(input.get_size_column(0));
	for(int i = 0; i < sortedTable.size();i++){
		auto& input_col = input.get_column(i);
		if (input_col.valid())
			sortedTable[i].create_gdf_column(input_col.dtype(), input_col.dtype_info(), input_col.size(), nullptr, ral::traits::get_dtype_size_in_bytes(input_col.dtype()), input_col.name());
		else
			sortedTable[i].create_gdf_column(input_col.dtype(), input_col.dtype_info(), input_col.size(), nullptr, nullptr, ral::traits::get_dtype_size_in_bytes(input_col.dtype()), input_col.name());
	}
...
void distributed_sort(Context & queryContext, blazing_frame& input, std::vector<gdf_column_cpp>& cols, std::vector<gdf_column*>& rawCols, std::vector<int8_t>& sortOrderTypes, std::vector<int>& sortColIndices) {
	using ral::communication::CommunicationData;
	static CodeTimer timer;
	timer.reset();
	...
	std::thread sortThread{[](Context & queryContext, blazing_frame& input, std::vector<gdf_column*>& rawCols, std::vector<int8_t>& sortOrderTypes, std::vector<gdf_column_cpp>& sortedTable){
		ral::config::GPUManager::getInstance().setDevice();
		static CodeTimer timer2;
		sort(queryContext, input, rawCols, sortOrderTypes, sortedTable);
		Library::Logging::Logger().logInfo(timer2.logDuration(queryContext, "distributed_sort part 2 async sort"));
		timer2.reset();
	}, std::ref(queryContext), std::ref(input), std::ref(rawCols), std::ref(sortOrderTypes), std::ref(sortedTable)};
	// sort(queryContext, input, rawCols, sortOrderTypes, sortedTable);
	...
	if (queryContext.isMasterNode(CommunicationData::getInstance().getSelfNode())) {
		...
		Library::Logging::Logger().logInfo(timer.logDuration(queryContext, "distributed_sort part 2 collectSamples generatePartitionPlans distributePartitionPlan"));
	}
	else {
		queryContext.incrementQuerySubstep();
		...
		Library::Logging::Logger().logInfo(timer.logDuration(queryContext, "distributed_sort part 2 sendSamplesToMaster getPartitionPlan"));
	}
	...
	Library::Logging::Logger().logInfo(timer.logDuration(queryContext, "distributed_sort part 4 distributePartitions collectPartitions"));
	timer.reset();
