...
cudf::reduction::operators gdf_agg_op_to_reduction_operators(const gdf_agg_op agg_op){
	switch(agg_op){
		...
		default:
			throw std::runtime_error("In gdf_agg_op_to_reduction_operators: Unexpected gdf_agg_op");
	}
}
...
cudf::groupby::operators gdf_agg_op_to_groupby_operators(const gdf_agg_op agg_op){
	switch(agg_op){
		case GDF_SUM:
			return cudf::groupby::SUM;
		case GDF_MIN:
			return cudf::groupby::MIN;
		case GDF_MAX:
			return cudf::groupby::MAX; 
		case GDF_COUNT:
			return cudf::groupby::COUNT; 
		case GDF_AVG:
			return cudf::groupby::MEAN; 
		case GDF_COUNT_DISTINCT:
			throw std::runtime_error("COUNT DISTINCT currently not supported with a group by");
		default: 
			throw std::runtime_error("Groupby Operator currently not supported with a group by");
...
std::vector<gdf_column_cpp> groupby_without_aggregations(std::vector<gdf_column_cpp>& input, const std::vector<int>& group_column_indices){
	...
	for(int i = 0; i < num_group_columns; i++){
		if (input[i].valid())
			grouped_output[i].create_gdf_column(input[i].dtype(), input[i].dtype_info(), index_col_ptr->size, nullptr, ral::traits::get_dtype_size_in_bytes(input[i].dtype()), input[i].name());
		else
			grouped_output[i].create_gdf_column(input[i].dtype(), input[i].dtype_info(), index_col_ptr->size, nullptr, nullptr, ral::traits::get_dtype_size_in_bytes(input[i].dtype()), input[i].name());
...
void distributed_groupby_without_aggregations(Context & queryContext, blazing_frame& input, std::vector<int>& group_column_indices){
	...
	Library::Logging::Logger().logInfo(timer.logDuration(queryContext, "distributed_groupby_without_aggregations part 0 generateSample"));						
	timer.reset();	
	...
	auto groupByTask = std::async(std::launch::async,
							[](Context & queryContext, std::vector<gdf_column_cpp>& input, const std::vector<int>& group_column_indices){
								...
								Library::Logging::Logger().logInfo(timer.logDuration(queryContext, "distributed_groupby_without_aggregations part 1 async groupby_without_aggregations"));						
								timer.reset();
								return result;
							},
	...
	if (queryContext.isMasterNode(CommunicationData::getInstance().getSelfNode())) {
		...
		Library::Logging::Logger().logInfo(timer.logDuration(queryContext, "distributed_groupby_without_aggregations part 1 collectSamples generatePartitionPlansGroupBy distributePartitionPlan"));						
		timer.reset();		
	}
	else {
		queryContext.incrementQuerySubstep();
		...
		Library::Logging::Logger().logInfo(timer.logDuration(queryContext, "distributed_groupby_without_aggregations part 1 sendSamplesToMaster getPartitionPlan"));						
		timer.reset();
	}
	...
	Library::Logging::Logger().logInfo(timer.logDuration(queryContext, "distributed_groupby_without_aggregations part 2 partitionData"));						
	timer.reset();
	...
	Library::Logging::Logger().logInfo(timer.logDuration(queryContext, "distributed_groupby_without_aggregations part 3 distributePartitions collectPartitions"));						
	timer.reset();

	ral::distribution::groupByWithoutAggregationsMerger(partitionsToMerge, group_column_indices, input);

	Library::Logging::Logger().logInfo(timer.logDuration(queryContext, "distributed_groupby_without_aggregations part 4 groupByWithoutAggregationsMerger"));						
	timer.reset();
}
...
std::vector<gdf_column_cpp> compute_aggregations(blazing_frame& input, std::vector<int>& group_column_indices, std::vector<gdf_agg_op>& aggregation_types, 
	...
	for(size_t i = 0; i < aggregation_types.size(); i++){
		...
		if (expression == "" && aggregation_types[i] == GDF_COUNT){ // this means we have a COUNT(*). So lets create a simple column with no nulls
			gdf_dtype_extra_info extra_info;
			extra_info.category = nullptr;
			extra_info.time_unit = TIME_UNIT_NONE;
			std::vector<int8_t> temp(row_size, 0);
			aggregation_inputs[i].create_gdf_column(GDF_INT8, extra_info, row_size, temp.data(), ral::traits::get_dtype_size_in_bytes(GDF_INT8), "");
		} else {
			if(contains_evaluation(expression)){
				...
				gdf_dtype_extra_info extra_info;
				extra_info.category = nullptr;
				extra_info.time_unit = (agg_input_type == GDF_DATE64 || agg_input_type == GDF_TIMESTAMP ? TIME_UNIT_ms : TIME_UNIT_NONE); // TODO this should not be hardcoded

				aggregation_inputs[i].create_gdf_column(agg_input_type, extra_info, row_size, nullptr, ral::traits::get_dtype_size_in_bytes(agg_input_type), "");
		...
		if( aggregation_column_assigned_aliases[i] == "" ){
			if (expression == "" && aggregation_types[i] == GDF_COUNT){ // COUNT(*) case
				...
				output_column_names[i] = aggregator_to_string(aggregation_types[i]) + "(" + aggregation_inputs[i].name() + ")";
			}
		}else{
...
void aggregationsMerger(std::vector<ral::distribution::NodeColumns>& aggregations, const std::vector<int>& groupColIndices, const std::vector<gdf_agg_op>& aggregationTypes, blazing_frame& output){
	...
	if (groupColIndices.size() == 0) {
		aggregations_without_groupby(modAggregationTypes, aggregation_inputs, output_columns_aggregations, aggregation_dtypes, aggregation_names);
	}else{
		aggregations_with_groupby(groupByColumns, aggregation_inputs, modAggregationTypes, group_by_output_columns, 
			output_columns_aggregations, aggregation_names);
	}
...
void distributed_aggregations_with_groupby(Context & queryContext, blazing_frame& input, std::vector<int>& group_column_indices, std::vector<gdf_agg_op>& aggregation_types, std::vector<std::string>& aggregation_input_expressions, std::vector<std::string>& aggregation_column_assigned_aliases) {
	using ral::communication::CommunicationData;
	static CodeTimer timer;
	timer.reset();
	...
	std::vector<gdf_column_cpp> selfSamples = ral::distribution::sampling::generateSample(group_columns, 0.1);
	Library::Logging::Logger().logInfo(timer.logDuration(queryContext, "distributed_aggregations_with_groupby part 0 generateSample"));						
	timer.reset();	

	auto aggregationTask = std::async(std::launch::async,
											[](Context & queryContext, blazing_frame& input, std::vector<int>& group_column_indices, std::vector<gdf_agg_op>& aggregation_types, std::vector<std::string>& aggregation_input_expressions, std::vector<std::string>& aggregation_column_assigned_aliases){
												ral::config::GPUManager::getInstance().setDevice();
												static CodeTimer timer2;
												std::vector<gdf_column_cpp> result = compute_aggregations(input, group_column_indices, aggregation_types, aggregation_input_expressions, aggregation_column_assigned_aliases);
												Library::Logging::Logger().logInfo(timer.logDuration(queryContext, "distributed_aggregations_with_groupby async compute_aggregations"));						
												timer.reset();
												return result;
											},
											std::ref(queryContext),
	...
	if (queryContext.isMasterNode(CommunicationData::getInstance().getSelfNode())) {
		...
		Library::Logging::Logger().logInfo(timer.logDuration(queryContext, "distributed_aggregations_with_groupby part 1 collectSamples generatePartitionPlansGroupBy distributePartitionPlan"));						
		timer.reset();
  }
	else {
		queryContext.incrementQuerySubstep();
		...
		queryContext.incrementQuerySubstep();
		partitionPlan = ral::distribution::getPartitionPlan(queryContext);
		Library::Logging::Logger().logInfo(timer.logDuration(queryContext, "distributed_aggregations_with_groupby part 1 sendSamplesToMaster getPartitionPlan"));						
		timer.reset();
	}
	...
	Library::Logging::Logger().logInfo(timer.logDuration(queryContext, "distributed_aggregations_with_groupby part 2 partitionData"));						
	timer.reset();
	...
	Library::Logging::Logger().logInfo(timer.logDuration(queryContext, "distributed_aggregations_with_groupby part 3 distributePartitions collectPartitions"));
	timer.reset();

	aggregationsMerger(partitionsToMerge, groupColumnIndices, aggregation_types, input);

	Library::Logging::Logger().logInfo(timer.logDuration(queryContext, "distributed_aggregations_with_groupby part 4 aggregationsMerger"));						
	timer.reset();
}

void distributed_aggregations_without_groupby(Context & queryContext, blazing_frame& input, std::vector<int>& group_column_indices, std::vector<gdf_agg_op>& aggregation_types, std::vector<std::string>& aggregation_input_expressions, std::vector<std::string>& aggregation_column_assigned_aliases) {
	using ral::communication::CommunicationData;
	static CodeTimer timer;
	timer.reset();
	...
	Library::Logging::Logger().logInfo(timer.logDuration(queryContext, "distributed_aggregations_without_groupby part 1 compute_aggregations"));
	timer.reset();
	...
	if (queryContext.isMasterNode(CommunicationData::getInstance().getSelfNode())) {
		...
		std::iota(groupColumnIndices.begin(), groupColumnIndices.end(), 0);
		aggregationsMerger(partitionsToMerge, groupColumnIndices, aggregation_types, input);
		Library::Logging::Logger().logInfo(timer.logDuration(queryContext, "distributed_aggregations_without_groupby part 2 collectPartitions aggregationsMerger"));
		timer.reset();
	}else{
		...
		Library::Logging::Logger().logInfo(timer.logDuration(queryContext, "distributed_aggregations_without_groupby part 2 distributePartitions"));
		timer.reset();
	}
}
