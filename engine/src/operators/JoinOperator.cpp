...
JoinOperator::JoinOperator(Context * context)
: context_{context} {
    ...
    for(int column_index = 0; column_index < input.get_size_columns(); column_index++){
        ...
        if(is_inner_join){
			if (input.get_column(column_index).valid())
				output.create_gdf_column(input.get_column(column_index).dtype(),input.get_column(column_index).dtype_info(),left_indices_.size(),nullptr,column_width, input.get_column(column_index).name());
			else
				output.create_gdf_column(input.get_column(column_index).dtype(), input.get_column(column_index).dtype_info(),left_indices_.size(),nullptr,nullptr,column_width, input.get_column(column_index).name());
		} else {
			if (!input.get_column(column_index).valid())
        ...
        if(left_indices_.size() != 0 && right_indices_.size() != 0){ // Do not materialize if the join output is empty
            ::materialize_column(input.get_column(column_index).get_gdf_column(),
                                ...
                                (column_index < first_table_end_index
                                    ? left_indices_.get_gdf_column()
                                    : right_indices_.get_gdf_column()));
        } else {
...
LocalJoinOperator::LocalJoinOperator(Context * context)
...
DistributedJoinOperator::DistributedJoinOperator(Context * context)
: JoinOperator(context)
{ }
...
blazing_frame DistributedJoinOperator::operator()(blazing_frame& frame, const std::string& query) {
    ...
    Library::Logging::Logger().logInfo(timer_.logDuration(*context_, "DistributedJoinOperator part 1 process_distribution"));
    timer_.reset();
    ...
    Library::Logging::Logger().logInfo(timer_.logDuration(*context_, "DistributedJoinOperator part 3 materialize_column"));
    timer_.reset();
...
blazing_frame DistributedJoinOperator::process_distribution(blazing_frame& frame, const std::string& query) {
    ...
    for (auto& column : tables[0]){
        if (column.dtype() == GDF_STRING_CATEGORY)
            row_width_bytes_left += ral::traits::get_dtype_size_in_bytes(column.dtype()) * 5; // WSM TODO. Here i am adding a fudge factor trying to account for the fact that strings can occupy a lot more. We needa better way to easily figure out how mmuch space a string column actually occupies
        else
            row_width_bytes_left += ral::traits::get_dtype_size_in_bytes(column.dtype());
    }
    for (auto& column : tables[1]){
        if (column.dtype() == GDF_STRING_CATEGORY)
            row_width_bytes_right += ral::traits::get_dtype_size_in_bytes(column.dtype()) * 5; // WSM TODO. Here i am adding a fudge factor trying to account for the fact that strings can occupy a lot more. We needa better way to easily figure out how mmuch space a string column actually occupies
        else
    ...
    size_t estimate_regular_distribution = (total_rows_left*row_width_bytes_left + total_rows_right*row_width_bytes_right)*(num_nodes - 1)/num_nodes;
    ...
    size_t MAX_SCATTER_MEM_OVERHEAD = 500000000; // 500Mb  how much extra memory consumption per node are we ok with  WSM TODO get this value from config
    if (estimate_scatter_left < estimate_regular_distribution || estimate_scatter_right < estimate_regular_distribution){
        if (estimate_scatter_left < estimate_scatter_right && total_rows_left*row_width_bytes_left < MAX_SCATTER_MEM_OVERHEAD){
            scatter_left = true;            
        } else if (estimate_scatter_right < estimate_scatter_left && total_rows_right*row_width_bytes_right < MAX_SCATTER_MEM_OVERHEAD){
            scatter_right = true;
        } 
    }
    ...
    if (scatter_left || scatter_right){
        ...
        if (scatter_left) {
            Library::Logging::Logger().logTrace(ral::utilities::buildLogString(std::to_string(context_->getContextToken()), std::to_string(context_->getQueryStep()), std::to_string(context_->getQuerySubstep()), "join process_distribution scatter_left"));
            ...
            Library::Logging::Logger().logTrace(ral::utilities::buildLogString(std::to_string(context_->getContextToken()), std::to_string(context_->getQueryStep()), std::to_string(context_->getQuerySubstep()), "join process_distribution scatter_right"));
        ...
        Library::Logging::Logger().logTrace(ral::utilities::buildLogString(std::to_string(context_->getContextToken()), std::to_string(context_->getQueryStep()), std::to_string(context_->getQuerySubstep()), "join process_distribution hash based distribution"));
        return process_hash_based_distribution(frame, query);
    }
}

blazing_frame DistributedJoinOperator::process_hash_based_distribution(blazing_frame& frame, const std::string& query) {
