...
cudf::io::table_with_metadata read_sqlite_v2(sqlite3_stmt * stmt,
	...
	const std::size_t num_words =
		bitmask_allocation / sizeof(cudf::bitmask_type);
	...
	std::transform(column_indices.cbegin(),
		...
		[&cudf_types, &null_masks, num_words, nRows](
		...
		for(const std::size_t projection_index : column_indices) {
			...
			case cudf::type_id::INT32: {
				const std::int32_t value =
					sqlite3_column_int(stmt, projection_index);
			...
			case cudf::type_id::INT64: {
				const std::int64_t value =
					sqlite3_column_int64(stmt, projection_index);
			...
			case cudf::type_id::DECIMAL64: {
				const double value =
					sqlite3_column_double(stmt, projection_index);
			...
			case cudf::type_id::STRING: {
				cudf_string_col * string_col =
					reinterpret_cast<cudf_string_col *>(
						host_cols[projection_index]);
				if(isNull) {
	...
	for(const std::size_t projection_index : column_indices) {
		...
		case cudf::type_id::STRING: {
			cudf_string_col * string_col = reinterpret_cast<cudf_string_col *>(
				host_cols[projection_index]);
	...
	for(const std::size_t projection_index : column_indices) {
		...
		case cudf::type_id::STRING: {
			delete reinterpret_cast<cudf_string_col *>(
				host_cols[projection_index]);
			break;
		}
