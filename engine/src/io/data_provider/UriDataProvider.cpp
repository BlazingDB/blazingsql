...
uri_data_provider::uri_data_provider(std::vector<Uri> uris)
 : data_provider(),  file_uris(uris), uri_scalars({}), string_scalars({}), is_column_string({}), opened_files({}), current_file(0), errors({}), directory_uris({}), directory_current_file(0) {

}

uri_data_provider::uri_data_provider(std::vector<Uri> uris,std::vector<std::map<std::string,gdf_scalar> > uri_scalars,std::vector<std::map<std::string,std::string> > string_scalars,std::vector<std::map<std::string,bool> > is_column_string)
	: data_provider(),  file_uris(uris), uri_scalars(uri_scalars), string_scalars(string_scalars), is_column_string(is_column_string), opened_files({}), current_file(0), errors({}), directory_uris({}), directory_current_file(0) {
    ...
    handle.uri = this->directory_uris[this->directory_current_file];
		...
		try {
			...
			if (fs_manager && fs_manager->exists(target_uri) ) {
				...
				throw std::runtime_error("Path '" + target_uri.toString() +
      ...
      if (hasWildcard) {
          ...
          this->directory_uris = BlazingContext::getInstance()->getFileSystemManager()->list(target_uri, wildcard);
