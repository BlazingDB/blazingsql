...
void data_loader::load_data(const Context & context, std::vector<gdf_column_cpp> & columns, const std::vector<size_t>  & column_indices, const Schema & schema){
	...
	for(int file_index = 0; file_index < files.size(); file_index++){
		threads.push_back(std::thread([&, file_index]()
		{
			...
			if(files[file_index].fileHandle != nullptr){
				...
				//std::cout<<"file is "<< user_readable_file_handles[file_index]<<" with uri "<<files[file_index].uri.getPath().toString()<<std::endl;
				Schema fileSchema = schema.fileSchema();
				parser->parse(files[file_index].fileHandle, user_readable_file_handles[file_index], converted_data,fileSchema,column_indices);
				...
				for(int i = 0; i < schema.get_num_columns(); i++){
					if(! schema.get_in_file()[i]){
						...
						if(files[file_index].is_column_string[name]){
							...
							gdf_column_cpp column;
							column.create_gdf_column(scalar.dtype,gdf_dtype_extra_info{TIME_UNIT_ms},num_rows,nullptr,ral::traits::get_dtype_size_in_bytes(scalar.dtype),name);
							cudf::fill(column.get_gdf_column(), scalar,
												0, num_rows);
							converted_data.push_back(column);
						}
				...
				Library::Logging::Logger().logError(ral::utilities::buildLogString("", "", "", "ERROR: Was unable to open " + user_readable_file_handles[file_index]));
			}
		}));
	...
	if(provider_errors.size() != 0){
		for(size_t error_index = 0; error_index < provider_errors.size(); error_index++){
			Library::Logging::Logger().logError(ral::utilities::buildLogString("", "", "", "ERROR: " + provider_errors[error_index]));
		}
	}
