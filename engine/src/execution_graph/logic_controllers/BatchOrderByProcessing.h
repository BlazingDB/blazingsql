	...
	void compute_partition_plan(std::vector<ral::frame::BlazingTableView> sampledTableViews, std::size_t avg_bytes_per_row, std::size_t local_total_num_rows) {
		if (this->context->getAllNodes().size() == 1){ // single node mode
			...
			if(context->isMasterNode(self_node)) {
				...
				partitionPlan = ral::operators::generate_partition_plan(samples, totalNumRows, final_avg_bytes_per_row, this->expression, this->context.get());
				...
				metadata.add_value(ral::cache::AVG_BYTES_PER_ROW_METADATA_LABEL, avg_bytes_per_row);
			...
			try {
				...
				for (auto i = 0; i < partitions.size(); i++) {
					...
					if(partition.first == self_node) {
						...
						bool added = this->add_to_output_cache(partition.second.clone(), cache_id,false);
						if (added) {
