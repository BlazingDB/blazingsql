...
struct tree_processor {
	...
	std::shared_ptr<kernel> make_kernel(std::string expr, std::shared_ptr<ral::cache::graph> graph) {
		std::shared_ptr<kernel> k;
		auto kernel_context = this->context->clone();
		if ( is_project(expr) ) {
			k = std::make_shared<Projection>(expr, kernel_context, graph);
			kernel_context->setKernelId(k->get_id());
			k->set_type_id(kernel_type::ProjectKernel);
		} else if ( is_filter(expr) ) {
			k = std::make_shared<Filter>(expr, kernel_context, graph);
			kernel_context->setKernelId(k->get_id());
			...
			k = std::make_shared<TableScan>(*loader, schema, kernel_context, graph);
			kernel_context->setKernelId(k->get_id());
