...
void CommunicationData::initialize(int unixSocketId, const std::string &orchIp,
  ...
  auto address = blazingdb::transport::Address::TCP(
      selfRalIp, selfRalCommunicationPort, selfRalProtocolPort);

  selfNode = blazingdb::transport::Node::Make(address);
}

const blazingdb::transport::Node &CommunicationData::getSelfNode() {
  return *selfNode;
}

std::shared_ptr<blazingdb::transport::Node> CommunicationData::getSharedSelfNode() {
  return selfNode;
}
