...
public:
  ...
  auto get_raw_pointers(gdf_column* column) {
    ...
    gdf_size_type stringsSize_ = std::accumulate(lengthPerStrings, lengthPerStrings + stringsLength_, 0,
    ...
    auto strDataTuple = std::make_tuple(stringsPointer_, stringsSize_, offsetsPointer_, offsetsSize_, nullBitmask_, nullMaskSize_);
  ...
  MakeFrom(const Message::MetaData &message_metadata,
    ...
    return std::make_shared<GPUComponentMessage>(
        message_metadata.messageToken,
        message_metadata.contextToken,
        node,
        received_samples,
        message_metadata.total_row_size);
  }
  std::vector<gdf_column_cpp> getSamples(){
    return samples;
  }
