...
struct SampleToNodeMasterMessage : GPUComponentMessage {
  ...
  SampleToNodeMasterMessage(const std::string& message_token,
                            ...
                            int total_row_size)
      : GPUComponentMessage(message_token,
                            context_token, sender_node, samples,
                            total_row_size) {}

  DefineClassName(SampleToNodeMasterMessage);

  std::size_t getTotalRowSize() const {
    return this->metadata().total_row_size;
  }
  ...
  MakeFrom(const Message::MetaData &message_metadata,
    ...
    return GPUComponentMessage::MakeFrom(message_metadata, address_metadata,
                                         columns_offsets, raw_buffers);
  }
};
...
struct ColumnDataMessage : GPUComponentMessage {
  ColumnDataMessage(const std::string& message_token,
                    ...
                    std::shared_ptr<Node> &sender_node,
                    std::vector<gdf_column_cpp> &samples)
      : GPUComponentMessage(message_token, context_token,
                            sender_node, samples) {}
