...
buffer_transport(metadata,buffer_sizes,column_transports),
transmitted_begin_frames(0), transmitted_frames(0), self_worker_id(self_worker_id) {

        auto endpoint_map = ral::communication::CommunicationData::getInstance().getWorkerToUcpEndpointMap();

		for(auto worker_id : StringUtil::split(metadata.get_values()[ral::cache::WORKER_IDS_METADATA_LABEL], ",")) {
			if(endpoint_map.find(worker_id) == endpoint_map.end()) {
				throw std::exception();	 // TODO: make a real exception here
			}
			destinations.push_back(endpoint_map[worker_id]);
		}
...
ucp_tag_t ucx_buffer_transport::generate_message_tag(){
    blazing_ucp_tag blazing_tag = { 
        message_id.fetch_add(1), self_worker_id, 0u};
...
void ucx_buffer_transport::send_begin_transmission(){
    ...
    {
        requests.push_back(ucp_tag_send_nb(address, buffer_to_send.data(), buffer_to_send.size(),
