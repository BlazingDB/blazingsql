...
cdef extern from "../include/engine/initialize.h" nogil:
    cdef bool initialize(int ralId, int gpuId, string network_iface_name, string ralHost, int ralCommunicationPort, bool singleNode, map[string,string] config_options) except +raiseInitializeError
    cdef void finalize() except +raiseFinalizeError
    cdef bool blazingSetAllocator(string allocation_mode, size_t initial_pool_size, map[string,string] config_options) except +raiseBlazingSetAllocatorError
