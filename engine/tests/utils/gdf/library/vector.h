...
private:
	...
	class InnerWrapper {
	public:
		InnerWrapper(Field<T> & container) : mField(container) {}

		T operator[](int index) { return mField[index]; }
...
public:
	...
	Vector & fillData(std::vector<Type> values) {
...
public:
	...
	int validSize() { return mValid.size(); }

	gdf_column * column() { return &mColumn; }
