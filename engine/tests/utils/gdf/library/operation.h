...
template <typename TypeOut, typename TypeVax, typename TypeVay>
struct TrueDiv {
	TypeOut operator()(TypeVax vax, TypeVay vay) { return (TypeOut)((double) vax / (double) vay); }
};

template <typename TypeOut, typename TypeVax, typename TypeVay>
struct FloorDiv {
	TypeOut operator()(TypeVax vax, TypeVay vay) { return (TypeOut) floor((double) vax / (double) vay); }
};
...
struct Mod<TypeOut, TypeVax, TypeVay, uint64_t> {
	TypeOut operator()(TypeVax x, TypeVay y) { return (TypeOut)((uint64_t) x % (uint64_t) y); }
};

template <typename TypeOut, typename TypeVax, typename TypeVay>
struct Mod<TypeOut, TypeVax, TypeVay, float> {
	TypeOut operator()(TypeVax x, TypeVay y) { return (TypeOut) fmod((float) x, (float) y); }
};

template <typename TypeOut, typename TypeVax, typename TypeVay>
struct Mod<TypeOut, TypeVax, TypeVay, double> {
	TypeOut operator()(TypeVax x, TypeVay y) { return (TypeOut) fmod((double) x, (double) y); }
};

template <typename TypeOut, typename TypeVax, typename TypeVay>
struct Pow {
	TypeOut operator()(TypeVax vax, TypeVay vay) { return (TypeOut) pow((double) vax, (double) vay); }
};
