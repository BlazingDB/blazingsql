...
gdf_error evaluate_query(std::vector<std::vector<gdf_column_cpp>> input_tables,
  ...
  blazing_frame output_frame = evaluate_split_query(
      input_tables, table_names, column_names, splitted, &queryContext);

  for (size_t i = 0; i < output_frame.get_width(); i++) {
    GDFRefCounter::getInstance()->deregister_column(
        output_frame.get_column(i).get_gdf_column());
    outputs.push_back(output_frame.get_column(i));
  }

  return GDF_SUCCESS;
}
} // namespace evaluate_query_wrapper
...
struct query_test : public ::testing::Test {
  ...
  gdf_error
  evaluate_query(std::vector<std::vector<gdf_column_cpp>> input_tables,
                 std::vector<std::string> table_names,
                 std::vector<std::vector<std::string>> column_names,
                 std::string logicalPlan,
                 std::vector<gdf_column_cpp> &outputs) {
    return interpreter_wrapper::evaluate_query(
        input_tables, table_names, column_names, logicalPlan, outputs);
  }
};