...
TEST_F(ExpressionTreeTest, case_1) {
  ...
  std::string expected = "+($0, FIRST_NON_MAGIC(MAGIC_IF_NOT(<($0, 5), FIRST_NON_MAGIC(MAGIC_IF_NOT(<($0, 2), 0), 1)), 2))";
...
TEST_F(ExpressionTreeTest, case_3) {
  ...
  std::string expected = "FIRST_NON_MAGIC(MAGIC_IF_NOT(<($0, 2), $0), FIRST_NON_MAGIC(MAGIC_IF_NOT(<($0, 5), *($0, 2)), +($0, 1)))";
  tree.transform_to_custom_op();
  EXPECT_EQ(tree.rebuildExpression(), expected);
}
