...
public:
	...
	void noArgFunc(){
		messages[0] += "noArgFunc" + std::to_string(this->x);
	}
	void oneArgValFunc(std::string arg){
		messages[1] += "oneArgValFunc" + arg;
	}
	void oneArgRefFunc(std::string & arg){
	...
	void oneArgConstValFunc(const std::string arg){
		messages[3] += "oneArgConstValFunc" + arg;
	}
	void oneArgConstRefFunc(const std::string & arg){
		messages[4] += "oneArgConstRefFunc" + arg;
	}
	void manyArgFunc(std::string arg, int num, std::string & refArg){
	...
	void createSubThread0(){
		threads[0] = BlazingThread(&TestClass::noArgFunc,this);
	}
	void createSubThread1(std::string msg){
		threads[1] = BlazingThread(&TestClass::oneArgValFunc,this, msg);
	}
	void createSubThread2(std::string & msg){
		threads[2] = BlazingThread(&TestClass::oneArgRefFunc,this, std::ref(msg));
	}
	void createSubThread3(std::string msg){
		threads[3] = BlazingThread(&TestClass::oneArgConstValFunc,this, msg);
	}
	...
	void functionWithErrorNoArgs(){
		throw BlazingException("A planned error NoArgs!");
	}
	...
	void functionThatCallsWithErrorWithArgs(int x, int y, int & z){
		...
		for(int i = 0; i < threads.size(); i++){
			int xi = x + i;
			threads[i] = BlazingThread([this,xi, y, &z]{
				TestClass::functionWithErrorWithArgs(xi, y, z);
			});
		}
		for(int i = 0; i < threads.size(); i++){
			threads[i].join();
...
TEST(BlazingThreadTest, BlazingThreadWithCaughtErrors) {
	...
	}
	{
		try{
			...
			for(int i = 0; i < threads.size(); i++){
				int xi = 1 + i;
				int y =2;
				threads[i] = BlazingThread([&test,xi, y, &z]{
					test.functionWithErrorWithArgs(xi, y, z);
				});
			}
			for(int i = 0; i < threads.size(); i++){
				threads[i].join();
	...
	}
	{
		try{
			...
			TestClass test = TestClass(123);
			int z = 0;
			BlazingThread thread = BlazingThread(&TestClass::functionThatCallsWithErrorWithArgs,&test, 1, 2, std::ref(z));
			thread.join();
