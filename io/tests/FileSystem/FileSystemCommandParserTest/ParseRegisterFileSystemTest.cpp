...
struct ValidRegisterFileSystemCommand {
	ValidRegisterFileSystemCommand(const std::string & command, const FileSystemEntity & expectedResult)
		: command(command), expectedResult(expectedResult) {}
...
const std::vector<ValidRegisterFileSystemCommand> validRegisterFileSystemCommands = {
	...
	ValidRegisterFileSystemCommand(
		"register local file system stored as 'local_disk1' root '/usr/share/bz/opt/'",  // input: valid register file
																						 // system command
	...
	// BEGIN HDFS
	ValidRegisterFileSystemCommand(
		"register hdfs file system stored as 'percy_hdfs1' with ('127.0.0.1', 54310, 'percy', 'LIBHDFS3', "
		"'KERBEROS_TICKET_sdfS') root '/user1/warehouse/spark/data/temp/'",  // input: valid register file system
																			 // command
		FileSystemEntity(  // expected result: valid FileSystemEntity instance
			"percy_hdfs1",
			FileSystemConnection(
				"127.0.0.1", 54310, "percy", HadoopFileSystemConnection::DriverType::LIBHDFS3, "KERBEROS_TICKET_sdfS"),
			Path("/user1/warehouse/spark/data/temp/", true))),
	ValidRegisterFileSystemCommand(
		"register hdfs file system stored as 'percy_hdfs22' with ('127.0.0.11', 54311, 'percyt', 'LIBHDFS3', "
		"'KERBEROS_TICKET_sdfS45')",  // input: valid register file system command
		FileSystemEntity(			  // expected result: valid FileSystemEntity instance
			"percy_hdfs22",
			FileSystemConnection("127.0.0.11",
				54311,
				"percyt",
	...
	// BEGIN S3
	ValidRegisterFileSystemCommand(
		"register s3 file system stored as 'bz_s3_testing' with ('bucket1', 'None', '', 'accessKeyIddsf3', "
		"'secretKey234', 'sessionToken3s3dfds') root '/root/tpch/tables/'",  // input: valid register file system
																			 // command
		FileSystemEntity(  // expected result: valid FileSystemEntity instance
			"bz_s3_testing",
			FileSystemConnection("bucket1",
	...
	ValidRegisterFileSystemCommand(
		"register s3 file system stored as 'simplicity_testing_encrypted' with ('stest_1', 'AES-256', '', "
		"'accessKey444', 'secretKeyAA', 'sessionTokenDDD') root '/files/dts/'",  // input: valid register file system
																				 // command
		FileSystemEntity(  // expected result: valid FileSystemEntity instance
			"simplicity_testing_encrypted",
			FileSystemConnection("stest_1",
			...
			Path("/files/dts/", true))),
	ValidRegisterFileSystemCommand(
		"register s3 file system stored as 'simplicity_testing_encrypted_kms' with ('buk123', 'AWS-KMS', "
		"'arn:aws:dasadasw', 'accessKeyO9', 'secretKey1A', 'sessionToken9') root '/__new_/files'",  // input: valid
																									// register file
																									// system command
		FileSystemEntity(  // expected result: valid FileSystemEntity instance
			"simplicity_testing_encrypted_kms",
			FileSystemConnection("buk123",
	...
	ValidRegisterFileSystemCommand(
		"register s3 file system stored as 'nm-s3-kms-no-root' with ('buk123', 'AWS-KMS', 'arn:aws:dasadasw', "
		"'accessKeyO9', 'secretKey1A', 'sessionToken9')",  // input: valid register file system command
		FileSystemEntity(								   // expected result: valid FileSystemEntity instance
			"nm-s3-kms-no-root",
			FileSystemConnection("buk123",
				S3FileSystemConnection::EncryptionType::AWS_KMS,
...
const std::vector<std::string> invalidRegisterFileSystemCommands = {"",
	...
	"register hdfs file system stored as 'percy_hdfs1' with ('EXTRA_ARG', '127.0.0.1', 54310, 'percy', 'LIBHDFS3', "
	"'KERBEROS_TICKET_sdfS') root '/user1/warehouse/spark/data/temp/'",
	"register hdfs file system stored as 'badd_root' with ('127.0.0.1', 54310, 'percy', 'LIBHDFS3', "
	"'KERBEROS_TICKET_sdfS') root '1 /asd asda / asdad/ a_0'",
	"register s3 file system stored as 'nm-s3-kms-no-root' with ('EXTRA_ARG', 'buk123', 'AWS-KMS', 'arn:aws:dasadasw', "
	"'accessKeyO9', 'secretKey1A', 'sessionToken9')"
	"register s3 file system stored as 'kms-EMPTY_ARN' with ('encrypted_bucket_kms1', 'AWS-KMS', '', 'accessKeyO9', "
	"'secretKey1A', 'sessionToken9')"
	"register s3 file system stored as 'kms-fail1' with ('encrypted_bucket_kms1', , , 'AWS-KMS', 'adasdd', "
	"'accessKeyO9', 'secretKey1A', 'sessionToken9')"
...
INSTANTIATE_TEST_CASE_P(ParseValidRegisterFileSystemCommandTestCase,
	ParseValidRegisterFileSystemCommandTest,
	testing::ValuesIn(validRegisterFileSystemCommands));
INSTANTIATE_TEST_CASE_P(ParseInvalidRegisterFileSystemCommandTestCase,
	ParseInvalidRegisterFileSystemCommandTest,
	testing::ValuesIn(invalidRegisterFileSystemCommands));
