...
struct PathToStringTestParam {
	PathToStringTestParam(const std::string &path,
        const std::string &expectedResourceName, const std::string &expectedFileExtension,
...
struct IsHasTestParam {
	IsHasTestParam(const std::string &path, bool expectedResult)
		: path(path),
		  expectedResult(expectedResult) {
	}
...
const std::vector<IsHasTestParam> hasTrailingSlashTestEntries = {
	IsHasTestParam(
		"/", // input
...
const std::vector<IsHasTestParam> hasWildcardTestEntries = {
	IsHasTestParam(
		"/data/", // input
		false // expected result
	),
