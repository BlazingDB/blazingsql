...
void NativeHdfs::connect(const std::string &host, int port, const std::string &user, arrow::io::HdfsDriver driver, const std::string &kerberosTicket) {
	...
	// Info: raise LIBNativeHdfs::NativeHdfs3 internal HdfsNetworkConnectException
		arrow::io::HadoopFileSystem::Connect(&config, &this->hdfs), "NativeHdfs::NativeHdfs Connect");
}
...
void NativeHdfs::disconnect() {
	this->failWhenNotOk(this->hdfs->Disconnect(), "NativeHdfs::NativeHdfs Disconnect");
}
...
void NativeHdfs::createFile(const char *path, const void *buffer, int64_t nbytes) const {
	...
	this->failWhenNotOk(file->Write(static_cast<const uint8_t *>(buffer), nbytes), "NativeHdfs::NativeHdfs Write no bytes");
...
bool NativeHdfs::exists(const char *path) const {
	return this->hdfs->Exists(path);
}
