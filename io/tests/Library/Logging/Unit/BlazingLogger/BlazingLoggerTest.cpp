...
Library::Logging::BlazingLogger Logger() {
    return Library::Logging::BlazingLogger();
}


TEST_F(BlazingLoggerTest, CheckLogger) {
    auto& mock = Library::Logging::ServiceLogging::getInstance().getMock();
    EXPECT_CALL(mock, setLogData(SafeMatcherCast<const std::string&>(StrEq(logData))))
               .Times(4);
...
TEST_F(BlazingLoggerTest, CheckLoggerInfo) {
    ...
    auto& mock = Library::Logging::ServiceLogging::getInstance().getMock();
    EXPECT_CALL(mock, setLogData(SafeMatcherCast<const std::string&>(MatchesRegex(regex))))
               .Times(4);
