...
struct Server {
	...
	unsigned int run() {
		...
		ip::tcp::acceptor acceptor(
			service, ip::tcp::endpoint(ip::address::from_string(host), (short) std::stoul(port)));
...
struct ClientSocket : public Library::Network::GenericSocket {
	...
	void setTotalBytesMessage(unsigned int value) { totalBytesMessage = value; }

	unsigned int getTotalBytesSent() { return totalBytesSent; }

	unsigned int getTotalPacketsSent() { return totalPacketsSent; }
...
TEST_F(LibraryLoggingPerformanceTest, PerformanceCoutOutput) {
	...
	std::cerr << "Logging Output Bandwidth: " << (double) totalBytesReceived / (double) loggingMean / 1000.0
			  << " MBps\n";
