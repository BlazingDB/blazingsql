...
namespace Library {
    namespace Logging {
        FileOutput::FileOutput(const std::string& filename, bool truncate) {
            ...
            if (file_exists && !truncate){ // if we are not truncating, lets make sure it does not get too big. If the file there is alread big, lets archive it. Will only keep one old one in archive
        ...
        void FileOutput::flush(const int nodeInd, const std::string& datetime, const std::string& level, const std::string& log) {
            std::unique_lock<std::mutex> lock(mutex);
