...
class GoogleCloudStorageOutputStream::GoogleCloudStorageOutputStreamImpl {
	public:
        ...
        GoogleCloudStorageOutputStreamImpl(const std::string &bucketName, const std::string &objectKey, std::shared_ptr<gcs::Client> gcsClient);
...
//		std::vector<Aws::GoogleCloudStorage::Model::CompletedPart> completedParts; //just an etag (for response) and a part number
		size_t currentPart;
		int64_t written;
};
...
GoogleCloudStorageOutputStream::GoogleCloudStorageOutputStreamImpl::GoogleCloudStorageOutputStreamImpl(const std::string &bucketName, const std::string &objectKey, std::shared_ptr<gcs::Client> gcsClient) {
...
//	request.SetKey(key);
//    Aws::GoogleCloudStorage::Model::CreateMultipartUploadOutcome createMultipartUploadOutcome = GoogleCloudStorageClient->CreateMultipartUpload(request);
//	if (createMultipartUploadOutcome.IsSuccess()) {
//		this->uploadId = createMultipartUploadOutcome.GetResult().GetUploadId();
//	} else {
//        Logging::Logger().logError("Failed to create Aws::GoogleCloudStorage::Model::CreateMultipartUploadOutcome for bucket: " + bucketName + " and key: " + objectKey);
...
//		}
//        throw BlazingGoogleCloudStorageException("Failed to create Aws::GoogleCloudStorage::Model::CreateMultipartUploadOutcome. Problem was " + createMultipartUploadOutcome.GetError().GetExceptionName() + " : " + createMultipartUploadOutcome.GetError().GetMessage());
...
arrow::Status GoogleCloudStorageOutputStream::GoogleCloudStorageOutputStreamImpl::write(const void* buffer, int64_t nbytes) {
...
//	written += nbytes;
//    Aws::GoogleCloudStorage::Model::UploadPartOutcome uploadOutcome = GoogleCloudStorageClient->UploadPart(uploadPartRequest);
...
//    Aws::GoogleCloudStorage::Model::CompleteMultipartUploadOutcome completeMultipartUploadOutcome = GoogleCloudStorageClient->CompleteMultipartUpload(completeMultipartUploadRequest);
...
GoogleCloudStorageOutputStream::GoogleCloudStorageOutputStream(const std::string &bucketName, const std::string &objectKey, std::shared_ptr<gcs::Client> gcsClient)
...
arrow::Status GoogleCloudStorageOutputStream::Close() {
...
arrow::Status GoogleCloudStorageOutputStream::Flush() {
	return this->impl_->flush();
}

arrow::Status GoogleCloudStorageOutputStream::Tell(int64_t* position) const {
