...
bool GoogleCloudStorage::Private::connect(const FileSystemConnection &fileSystemConnection) {
    ...
    const std::string useDefaultAdcJsonFile = fileSystemConnection.getConnectionProperty(ConnectionProperty::USE_DEFAULT_ADC_JSON_FILE);
    ...
    if (!opts) {
        const std::string error = "Couldn't create gcs::ClientOptions for Project ID " + projectId + " status=" + opts.status().message();
...
bool GoogleCloudStorage::Private::exists(const Uri &uri) const {
    ...
    //TODO here we are removing the first "/" char so we create a Google Cloud Storage object key using the path ... improve this code
    ...
    if (objectMetadata) { // if success
        ...
        if ((contentLength == SIZE_OF_OBJECT_DIRECTORY) || (contentLength == 0)) { // may be a directory
            if (contentType == "text/plain") { // may be a text file
                ...
                // WARNING percy this test can fail if you have a text file with the word "placeholder" inside of it ... TODO
...
std::vector<Uri> GoogleCloudStorage::Private::list(const Uri &uri, const std::string &wildcard) const {
    ...
    if (objectsOutcome.begin() != objectsOutcome.end()) {
        ...
        for (auto&& object_metadata : objectsOutcome) {
            //WARNING TODO percy there is no folders concept in S# ... we should change Path::isFile::bool to Path::ObjectType::Unkwnow,DIR,FILE,SYMLIN,ETC
            const Path path = Path("/" + object_metadata->name(), true).getPathWithNormalizedFolderConvention(); //TODO percy avoid hardcoded string
...
std::vector<std::string> GoogleCloudStorage::Private::listResourceNames(const Uri &uri, FileType fileType, const std::string &wildcard) const {
...
//		}
//	} else {
//		Logging::Logger().logError("GoogleCloudStorage::Private::listResourceNames failed for URI: " + uri.toString());
//		bool shouldRetry = objectsOutcome.GetError().ShouldRetry();
//		if (shouldRetry){
//			Logging::Logger().logError(objectsOutcome.GetError().GetExceptionName() + " : " + objectsOutcome.GetError().GetMessage() + "  SHOULD RETRY");
//		} else {
...
std::vector<std::string> GoogleCloudStorage::Private::listResourceNames(const Uri &uri, const std::string &wildcard) const {
    std::vector<std::string> response;
...
//    }
    ...
    if (object_metadata) { // if success
        ...
        // TODO percy
//        Logging::Logger().logError("GoogleCloudStorage::Private::makeDirectory failed for URI: " + uri.toString());
...
//        }
        ...
        throw BlazingS3Exception("Could not make directory " + uriWithRoot.toString() + ". Problem was " + result.message());
    }
...
//		}
//	} else {
//		Logging::Logger().logError("GoogleCloudStorage::Private::move failed for src URI: " + src.toString() + "  and dst UTI: " + dst.toString());
//		bool shouldRetry = result.GetError().ShouldRetry();
//		if (shouldRetry){
//			Logging::Logger().logError(result.GetError().GetExceptionName() + " : " + result.GetError().GetMessage() + "  SHOULD RETRY");
//		} else {
...
bool GoogleCloudStorage::Private::openReadable(const Uri &uri, std::shared_ptr<GoogleCloudStorageReadableFile>* file) const {
    if (uri.isValid() == false) {
        throw BlazingInvalidPathException(uri);
    }
...
bool GoogleCloudStorage::Private::openWriteable(const Uri &uri, std::shared_ptr<GoogleCloudStorageOutputStream>* file) const {
//	if (uri.isValid() == false) {
//		throw BlazingInvalidPathException(uri);
//	}
