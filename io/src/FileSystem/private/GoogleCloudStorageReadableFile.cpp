...
GoogleCloudStorageReadableFile::GoogleCloudStorageReadableFile(std::shared_ptr<gcs::Client> gcsClient, std::string bucketName, std::string key) {
	this->key = key;
...
arrow::Status GoogleCloudStorageReadableFile::Read(int64_t nbytes, int64_t* bytesRead, void* buffer) {
    ...
    if (!results.status().ok()) {
        ...
        Logging::Logger().logWarn("GoogleCloudStorageReadableFile::Read, GetObject failed for bucketName: " + bucketName + " key " + key);
...
//        }
        ...
        */
        ...
        position += bytesRead;
        //if (bytesRead < nbytes){
        //    Logging::Logger().logError("Did not read all the bytes at GoogleCloudStorageReadableFile::ReadAt(int64_t position, int64_t nbytes, std::shared_ptr<arrow::Buffer>* out)");
        //}
...
arrow::Status GoogleCloudStorageReadableFile::ReadAt(int64_t position, int64_t nbytes, int64_t* bytesRead, void* buffer) {
...
//	std::cout<<"GoogleCloudStorageReadableFile::ReadAt " + std::to_string(nbytes)<<std::endl;
    ...
    if (!results.status().ok()) {
        ...
        Logging::Logger().logWarn("GoogleCloudStorageReadableFile::ReadAt, GetObject failed for bucketName: " + bucketName + " key " + key);
...
arrow::Status GoogleCloudStorageReadableFile::ReadAt(int64_t position, int64_t nbytes, std::shared_ptr<arrow::Buffer>* out) {
...
bool GoogleCloudStorageReadableFile::supports_zero_copy() const {
	return false;
}
