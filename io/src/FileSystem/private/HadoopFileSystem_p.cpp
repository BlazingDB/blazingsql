...
std::vector<Uri> HadoopFileSystem::Private::list(const Uri &uri, const std::string &wildcard) const {
	...
	if (status.ok()) {
        ...
        const std::string host = this->fileSystemConnection.getConnectionProperty(HadoopFileSystemConnection::ConnectionProperty::HOST);
        const std::string port = this->fileSystemConnection.getConnectionProperty(HadoopFileSystemConnection::ConnectionProperty::PORT);
