 ...
 *
 * Returns a FileSystemEntity if the command is correct, otherwise fill the error argument and return invalid
 * FileSystemEntity.
 *
 * @note
 * The command has the next syntax:
 * register [local/hdfs/s3] file system stored as 'namespaceId' with ('connectionPropertyValue1',
 * 'connectionPropertyValue2', ... , 'connectionPropertyValueN') [root 'optionalRootPath'] Specific syntax for each File
 * System: register local file system stored as 'namespaceId' root 'optionalRootPath' register hdfs file system stored
 * as 'namespaceId' with (host, port, user, driver, kerberos_ticket) root 'optionalRootPath' register s3 file system
 * stored as 'namespaceId' with (bucket, encryption_type, kms_key_amazon_resource_name, access_key_id, secret_key,
 * session_token) root 'optionalRootPath'
 *
